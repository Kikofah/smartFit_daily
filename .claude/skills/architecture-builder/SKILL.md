...
- ข้อยกเว้นที่ 1: การอ้างอิงชื่อระบบภายนอกที่ business rule กำหนดไว้แล้วจริงๆ ใน `01-spec/` (เช่น "YouTube"
...
- ข้อยกเว้นที่ 2 (ใหม่ — ยืนยันจากผู้ใช้ 2026-08-28): **section "ภาคผนวก: Stack Mapping" ท้ายเอกสาร**
  (ดูขั้นตอนที่ 3 ข้อ 11) อนุญาตให้มีชื่อเทคโนโลยีจริงจาก `tech-stack.md` ได้ — เป็น section เดียวในทั้ง
  เอกสารที่ยกเว้นกติกานี้ เนื้อหาหลัก (หัวข้อ 1-10) ยังคง conceptual ล้วนเหมือนเดิมทุกประการ ไม่ปะปนกัน
...
- `docs/02-design/02-technical/tech-stack.md` ถูกสร้าง/แก้ไข (ถ้ามี) — ต้องตรวจว่า "ภาคผนวก: Stack
  Mapping" ของเอกสารนี้ยัง sync กับ mapping ล่าสุดใน `tech-stack.md` § Mapping จาก Conceptual Docs →
  Concrete Stack หรือไม่
...
5. **Self-check กติกา conceptual**: อ่านเอกสารตัวเองซ้ำ ตรวจว่าหัวข้อ 1-10 (เนื้อหาหลัก) มีคำศัพท์ผูก
   stack หลุดเข้ามาหรือไม่ (เช่น ชื่อ framework/DB/cloud ที่ไม่ใช่ข้อยกเว้นตามกติกาข้างบน) — ถ้าเจอ ถือเป็น
   สิ่งที่ต้องแก้ทันทีไม่ต้องถามผู้ใช้ (เป็นการรักษากติกาเดิมของเอกสาร ไม่ใช่การตัดสินใจเนื้อหาใหม่) — ไม่รวม
   หัวข้อ 11 (ภาคผนวก: Stack Mapping) ซึ่งได้รับอนุญาตให้มีชื่อจริง
6. **Stack Mapping Appendix freshness** (ถ้ามี `tech-stack.md` และเอกสารนี้มีหัวข้อ 11 อยู่แล้ว): เทียบ
   เนื้อหาในหัวข้อ 11 กับ `tech-stack.md` § Mapping จาก Conceptual Docs → Concrete Stack ปัจจุบัน — ถ้า
   ไม่ตรงกัน (เช่น component ใหม่ที่ยังไม่ได้ map, ชื่อ implementation เปลี่ยนไปเพราะ `tech-stack.md`
   อัปเดต) ถือเป็น **การมิเรอร์ข้อเท็จจริงที่ตัดสินใจแล้วที่อื่น ไม่ใช่การตัดสินใจเนื้อหาใหม่** — แก้ให้ตรง
   ได้เองผ่าน flow ปกติ ไม่ต้องถามผู้ใช้ (ต่างจาก drift ในหัวข้อ 1-10 ที่อาจต้องถามตามกติกาปกติ)
...
10. **ความสัมพันธ์กับเอกสารอื่น** — ลิงก์กลับ `backlog.md`, `01-spec/*.md` ที่เกี่ยวข้อง, `user-journeys.md`,
    ...
    นี้)
11. **ภาคผนวก: Stack Mapping** (สร้าง/อัปเดตเฉพาะเมื่อ `docs/02-design/02-technical/tech-stack.md`
    มีอยู่แล้ว — ถ้ายังไม่มีให้ข้ามหัวข้อนี้ไปทั้งหมด ไม่ใช่ gap) — มิเรอร์ (ไม่ใช่แก้ไข) เนื้อหาจาก
    `tech-stack.md` § Mapping จาก Conceptual Docs → Concrete Stack § 6.1 เฉพาะส่วนที่เกี่ยวกับ HLA:
    ตาราง Conceptual Component (หัวข้อ 4) → concrete implementation จริงที่เลือกใช้ พร้อมประโยคเปิดที่
    ระบุชัดว่า **"หัวข้อนี้เป็นข้อยกเว้นเดียวในเอกสารนี้ที่มีชื่อเทคโนโลยีจริง แหล่งที่มาและสิทธิ์แก้ไขจริง
    อยู่ที่ [tech-stack.md](tech-stack.md) เสมอ — ถ้าทีมเปลี่ยน stack ในอนาคต ให้รัน `tech-stack-builder`
    ก่อน แล้วภาคผนวกนี้จะถูก sync ตามในการรัน `architecture-builder` ครั้งถัดไป"**
...
- ทุก Mermaid diagram ในหัวข้อ 1-10 ต้องเป็นระดับแนวคิด (ชื่อ node เป็นหน้าที่/ข้อมูล ไม่ใช่ชื่อ
  service/technology จริง) — ไม่บังคับกับหัวข้อ 11 (ภาคผนวก) ซึ่งเป็นข้อยกเว้น
- ตรวจว่า `docs/02-design/02-technical/tech-stack.md` มีอยู่หรือไม่ก่อนเขียนหัวข้อ 11 — มีก็สร้าง/อัปเดต
  ภาคผนวกให้ตรงกับ mapping ล่าสุด ไม่มีก็ข้ามหัวข้อนี้ไปทั้งหมด (ไม่ต้องสร้างหัวข้อเปล่า)
...
- ไฟล์ที่สร้าง/แก้ไข (`high-level-architecture.md`, `index.md` ถ้าแก้, log entry) และระบุว่าหัวข้อ 11
  (ภาคผนวก: Stack Mapping) ถูกสร้าง/อัปเดต/ข้าม (เพราะยังไม่มี `tech-stack.md`) ด้วยเหตุผลอะไร
