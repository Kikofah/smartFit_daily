...
1. **กติกาที่เข้มงวดที่สุด**: ห้ามระบุชื่อ framework/library/ฐานข้อมูล/cloud provider/ภาษาโปรแกรม/รูปแบบ API
   เฉพาะเจาะจงใดๆ ในเอกสาร ใช้คำเชิงหน้าที่แทนเสมอ (เช่น "mobile client" แทนชื่อ framework, "แหล่งเก็บข้อมูล
   เชิงโครงสร้าง" แทนชื่อฐานข้อมูล) ข้อยกเว้นเดียวคือชื่อระบบภายนอกที่ `01-spec/` ระบุไว้เป็นข้อเท็จจริงของ
   requirement เองอยู่แล้ว (เช่น YouTube, Apple Health/Google Health Connect) ซึ่งต้องอธิบายในฐานะ
   **external integration boundary** ไม่ใช่ stack ที่ทีมเลือก ถ้าตรวจพบว่าตัวเองกำลังเขียนชื่อ stack ใดๆ
   ลงไป ให้แก้เป็นระดับแนวคิดทันทีโดยไม่ต้องถามผู้ใช้ (เป็นการรักษากติกาเดิม ไม่ใช่การตัดสินใจเนื้อหาใหม่)
...
8. **เอกสารเป็นไฟล์เดียว ไม่ versioned** — อัปเดตทับไฟล์เดิมได้เลย ไม่ต้องถามเรื่อง version folder แบบ
   prototype
9. หลังสร้าง/แก้ไฟล์แล้ว อัปเดต `docs/02-design/02-technical/index.md` ให้กล่าวถึงไฟล์นี้ (ถ้ายังไม่ได้กล่าว
