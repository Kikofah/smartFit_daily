...
- ~~INT-1: จำนวนวันขั้นต่ำก่อนเริ่มพยากรณ์ได้~~ — **resolve แล้ว 2026-09-26**: ต้องมี log อย่างน้อย 7 วัน
  (น้อยกว่า 7 → empty state, ครบ 7 พอดี → พยากรณ์ได้) ตาม
  ...
  — `10-progress-insights.html` ปรับ empty state ให้ระบุ 7 วันตรง ๆ แล้ว (สอดคล้อง AC-INT-1-02)
...
1. **เพิ่มหน้าจอใหม่ 4 หน้า** (`00-auth-welcome.html`, `00-auth-signup.html`, `00-auth-login.html`,
   ...
   - `00-auth-login.html` (REQ-15, REQ-16): *(ย่อหน้านี้ถูกแทนที่แล้วในส่วนที่เกี่ยวกับ login ไม่สำเร็จ — ดู
     "เปลี่ยนแปลงจาก audit 2026-09-26" ท้ายไฟล์)* เหมือน signup แต่มีลิงก์ "ลืมรหัสผ่าน?" — ตั้งใจ **ไม่ทำ** UI
...
## เปลี่ยนแปลงจาก audit (Prototype Consistency Audit, 2026-09-26 — ONB-0 เข้าสู่ระบบไม่สำเร็จ)

เอกสารต้นทางเพิ่มกรณี **เข้าสู่ระบบไม่สำเร็จ (REQ-15)** เมื่อ 2026-09-26: decision ใน
[Onboarding spec](../../../01-requirements/01-spec/20260823-01-onboarding-personalization.md#ข้อสมมติฐานการตัดสินใจที่ยืนยันแล้ว),
[user-journeys.md § ONB-0](../user-journeys.md#onb-0--สมัครสมาชิก--เข้าสู่ระบบ--ลืมรหัสผ่าน--ออกจากระบบ-req-14-req-15-req-16-req-17)
Step 10 + Alt/Edge Case, AC-ONB-0-08/09/10 และ TC-ONB-0-008 ถึง 011 — audit `00-auth-login.html` เทียบ 7 ชั้น
พบว่าเป็น **prototype ล้าหลัง** เท่านั้น (ไม่มีข้อขัดแย้ง): หมายเหตุเดิมที่ว่า "ตั้งใจไม่ทำ UI อีเมล/รหัสผ่านไม่
ถูกต้อง" ไม่จริงอีกต่อไป AC-ONB-0-09 (ช่องว่าง) ตรงอยู่แล้ว ส่วน AC-ONB-0-08/10 ยังไม่มีสถานะให้เห็น ผู้ใช้
ยืนยันให้แก้ใน `v1/` (ไม่สร้าง `v2/`) และเลือกทั้ง 2 content decision ตามที่แนะนำ:

1. **ตำแหน่งข้อความ error ของทั้งฟอร์ม**: ข้อความ `--color-danger` ขนาด caption **เหนือปุ่ม "เข้าสู่ระบบ"**
   โดยไม่เปลี่ยนกรอบช่องใดเป็นสีแดง เพราะข้อความนี้ตั้งใจไม่บอกว่าผิดที่ช่องไหน — ใช้ token ที่มีอยู่แล้วใน
   DESIGN.md ไม่เพิ่ม component ใหม่ และตรงกับตำแหน่งในเว็บแอปจริง (`apps/web` `LoginScreen.tsx`)
2. **การจำลอง credential**: เฉพาะบัญชีทดลอง `newuser@example.com` / `Passw0rd!23` (test data ของ
   TC-ONB-0-002) ที่ผ่าน ค่าอื่นทั้งหมดได้ "อีเมลหรือรหัสผ่านไม่ถูกต้อง" มีบรรทัดเล็กสี `--color-ink-faint`
   บอกบัญชีทดลองใต้ปุ่ม — เปลี่ยนพฤติกรรมเดิมที่กรอกอะไรก็ได้แล้วผ่าน

สถานะที่เพิ่ม: ช่องว่าง → ข้อความใต้ช่อง (เดิม), รหัสผ่านผิด/อีเมลที่ไม่มีบัญชี → "อีเมลหรือรหัสผ่านไม่ถูกต้อง",
รูปแบบอีเมลผิด → "รูปแบบอีเมลไม่ถูกต้อง", ออฟไลน์ (รวมปุ่ม Google/Apple) → "เชื่อมต่ออินเทอร์เน็ตไม่ได้
กรุณาตรวจสอบการเชื่อมต่อแล้วลองใหม่" — ข้อความของบัญชีถูกระงับ/ลองผิดหลายครั้ง/pop-up Google มีในแอปจริงแต่
จำลองใน prototype ไม่ได้อย่างมีความหมาย จึงไม่ได้ใส่ ตรวจในเบราว์เซอร์แล้วครบทั้ง 6 กรณี (ช่องว่าง, รหัสผ่านผิด,
อีเมลที่ไม่มีบัญชี, รูปแบบผิด, ออฟไลน์, บัญชีทดลองเข้าได้)
