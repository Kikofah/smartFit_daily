...
- **อัปเดตล่าสุด (INT-1 เกณฑ์ log ขั้นต่ำ):** 2026-09-26 — narrow scope เฉพาะ INT-1: sync ตามการตัดสินใจ
  ที่ผู้ใช้งานยืนยันแล้วใน [Smart Integrations](../../01-requirements/01-spec/20260823-04-smart-integrations.md)
  ("ข้อสมมติฐาน/การตัดสินใจที่ยืนยันแล้ว") ว่า INT-1/REQ-11 ต้องมี log สะสมอย่างน้อย 7 วันก่อนพยากรณ์ได้
  (merely-stale drift ไม่ขัดแย้ง) — แก้ Flow 5 (§4.5) node I4 จาก "มี log เพียงพอ?" เป็น "มี log ≥ 7 วัน?"
  พร้อมคำบรรยาย INT-1 Step 4-5 และปิดหัวข้อ 8 ข้อ 3 เป็น resolved (ไม่แตะส่วนอื่นของเอกสาร)
- **อัปเดตก่อนหน้า (status-text fix, Stack Mapping):** 2026-08-31 — แก้แถว **Content Recommendation** ใน
...
- **INT-1** (Step 1-9): เปิดหน้า Progress/Insights → ตรวจว่ามีน้ำหนักเป้าหมายจาก ONB-3 หรือไม่ (Step 2,
  ดูหัวข้อ 8.1) → ไม่มี: แจ้งให้ไปกรอกก่อน (Step 3) → มี: ตรวจว่ามี log แคลอรี่สะสมอย่างน้อย 7 วันหรือไม่ (Step 4,
  ดูหัวข้อ 8 ข้อ 3) → น้อยกว่า 7 วัน: แจ้งให้สะสมข้อมูลเพิ่ม (Step 5) → ครบ 7 วันขึ้นไป: ดึงอัตราขาดดุลเฉลี่ยจาก log จริงของ Flow 4 (Step 6) →
...
3. ~~**INT-1/REQ-11** (Insights & Forecast): ยังไม่ระบุจำนวนวัน log ขั้นต่ำก่อนเริ่มพยากรณ์ได้ — กระทบเงื่อนไข
   "มี log เพียงพอ" ใน Flow 5~~ — **resolve แล้ว 2026-09-26**: ต้องมี log แคลอรี่สะสม**อย่างน้อย 7 วัน**
   ก่อนเริ่มพยากรณ์ได้ (ผู้ใช้งานยืนยันแล้ว บันทึกเป็นแหล่งที่มาจริงใน
   [Smart Integrations — ข้อสมมติฐาน/การตัดสินใจที่ยืนยันแล้ว](../../01-requirements/01-spec/20260823-04-smart-integrations.md))
   — Flow 5 (§4.5) node I4 และคำบรรยาย INT-1 Step 4-5 อัปเดตเป็นเงื่อนไข "มี log ≥ 7 วัน" แล้ว
