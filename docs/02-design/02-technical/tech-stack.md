...
- **อัปเดตล่าสุด:** 2026-08-31 (รอบ 4, factual correction) — `detailed-design-writer` audit
  ...
  **streak walk-back (PLN-4)** อยู่ในกลุ่ม "Client-side calculation ตาม NFR-01/NFR-03" เหมือน TDEE/safety
  ...
  "อัปเดตก่อนหน้านั้นอีกที") — ยืนยันจากโค้ดจริง `apps/web/server/routes/logging-streak/
  recomputeStreak.ts`: เป็นฟังก์ชัน**server-side ล้วน** ที่ loop query Firestore ตรงบน server เอง ไม่มี
  client เกี่ยวข้องเลย ถูกเรียกอัตโนมัติจาก route อื่น (`exertion-calorie`, `planner-day-status`) หลังเขียน
  `dailyLogs` เสร็จ ไม่ใช่รับผลคำนวณที่ client ทำมาก่อนแล้วส่งมาบันทึก — **ตัด "streak walk-back (PLN-4)"
  ออกจากรายการ "Client-side calculation ตาม NFR-01/NFR-03" ในหัวข้อ 4** พร้อมเพิ่มหมายเหตุอธิบายเหตุผล
  (server-side execution ไม่มีข้อจำกัด latency แบบเดียวกับ REC-2's MET calculation ที่ต้องการ feedback
  sub-250ms ตาม NFR-02 ระหว่างออกกำลังกายจริง) — factual correction ของ implementation ที่ ship แล้วจริง
  ไม่ใช่การเปลี่ยนตัวเลือก stack จริงจุดใด จึงไม่ใช้ AskUserQuestion — ดู log
  [2026-08-31](../../05-log/20260831-log.md)
- **อัปเดตก่อนหน้า:** 2026-08-31 (รอบ 3, factual correction) — แก้แถว **Content Recommendation** ในหัวข้อ
  6.1 ที่ยังระบุว่า `POST /api/workouts/sessions` "ยังเป็น TODO ในโค้ดจริง" ไม่เขียน embedded array field
  `sessionVideos: []` — แต่โค้ดจริง (`apps/web/server/routes/content-recommendation/index.ts`, commit
  `100bbd3`) implement แล้ว: เขียน 1 รายการ (role `main`) ตามปกติ หรือ 3 รายการ (`warmup`/`main`/`cooldown`
  ใช้ `externalVideoId` เดียวกัน) เมื่อ intensity เป็น `high` — ตรงกับ REC-1/REC-4's algorithm ใน
  `detailed-design/02-daily-youtube-recommendation.md` ทุกประการอยู่แล้ว (ไม่มี algorithm drift) เป็นการ
  แก้ status-text ล้วนๆ ไม่ใช่การเปลี่ยนตัวเลือก stack — ดู log [2026-08-31](../../05-log/20260831-log.md)
- **อัปเดตก่อนหน้านั้น:** 2026-08-31 (รอบ 2, factual correction) — `api-db-spec-writer` flag ว่าแถว
  **Personalization & Profile** ในหัวข้อ 6.1 ล้าหลังจากการแก้ `database-schema.md` §3.3
  (`goal_selection`)/`api-spec.md`'s `PUT /profile/goal` ที่ reinstate ฟิลด์ `dailyIntakeTargetKcal`
  แล้ว: เดิมแถวนี้ระบุ `goalSelection` มีแค่ 4 field (`goalType`/`targetWeightKg`/
  `dailyCalorieTargetKcal`/`isSafetyFloorApplied`) และเข้าใจผิดว่า safety floor gate ที่
  `dailyCalorieTargetKcal <= 1200` — **แก้ให้ตรงกับ design จริงที่ ship แล้ว**: `goalSelection` มี 5
  field ครบ (เพิ่ม `dailyIntakeTargetKcal`), `dailyCalorieTargetKcal` (เป้าหมายเผาผลาญจากออกกำลังกาย =
  น้ำหนักตัว × ค่าคงที่ตามเป้าหมาย) **ไม่มี safety floor**, safety floor gate เฉพาะ
  `dailyIntakeTargetKcal` (TDEE ± ค่าส่วนต่างตามเป้าหมาย ปรับขึ้นเป็น `SAFETY_FLOOR_MIN_KCAL` = 1,200
  kcal เมื่อต่ำกว่า) เท่านั้น — เป็นการแก้ข้อเท็จจริงของ design ที่ยืนยัน/ship ไปแล้ว ไม่ใช่การเปลี่ยน
  ตัวเลือก stack จริงจุดใด จึงไม่ต้องรัน Discovery Questionnaire ซ้ำ — ตรวจ §4 (Rationale) แล้วไม่พบข้อความ
  ที่ระบุ field/threshold ผิดซ้ำอีก (มีแค่ชื่อฟีเจอร์ "safety floor" เฉยๆ ไม่ต้องแก้) — ดู log
  [2026-08-31](../../05-log/20260831-log.md)
- **อัปเดตก่อนหน้า:** 2026-08-31 (รอบ 1) — **Tech Stack Consistency Audit + Reconciliation** ต่อจากที่
...
- **Client-side calculation ตาม NFR-01/NFR-03**: [Detailed Design](detailed-design/) ระบุชัดว่า TDEE
  (ONB-1), safety floor (ONB-3), และ MET+wearable override (REC-2) ต้องเป็นการคำนวณที่ไม่มี network
  latency — **แก้ไข 2026-08-31**: เดิมรายการนี้เคยรวม forecast (INT-1) ด้วย แต่ `detailed-design-builder`
  audit พบว่าเป็นการเข้าใจผิดจากการ generalize มาจาก REC-2's MET calculation ที่มีเหตุผลด้าน latency
  ระหว่างออกกำลังกายจริงชัดเจน (ต้องการ feedback ภายใน 250ms ตาม NFR-02) — **Forecast (INT-1) ไม่มีข้อจำกัด
  latency แบบเดียวกันเลย**: เป็น bodyless `GET /api/insights/forecast` ที่คำนวณทั้งหมดฝั่ง server
  (`apps/web/server/routes/insights-forecast/index.ts`) ไม่มี client-side precomputation ใดๆ ตั้งแต่แรก
  (ดูหัวข้อ 6.1 แถว Insights & Forecast) — ตัดออกจากรายการนี้ให้ตรงกับ
  `detailed-design/04-smart-integrations.md` ฉบับล่าสุดแล้ว — **แก้ไขเพิ่มเติม 2026-08-31 (รอบ 3,
  factual correction)**: รายการนี้เคยรวม **streak walk-back (PLN-4)** ด้วยเช่นกัน แต่พบข้อผิดพลาดแบบเดียวกัน
  ทุกประการ — `apps/web/server/routes/logging-streak/recomputeStreak.ts` เป็นฟังก์ชัน**server-side ล้วน**
  ที่ loop query Firestore ตรงบน server เอง (`for (let offset = 0; ...) { const log = await
  db.doc(...).get(); ... }`) ถูกเรียกอัตโนมัติจากทุก route ที่เขียน `dailyLogs`/`dayStatus`
  (`exertion-calorie`, `planner-day-status`) ทันทีหลังเขียนเสร็จ ไม่มี client ฝั่งใดคำนวณหรือส่งผลลัพธ์
  ล่วงหน้ามาก่อนแต่อย่างใด — เหมือนกรณี Forecast (INT-1) ทุกประการ: PLN-4 ไม่มีข้อจำกัด sub-250ms feedback
  ระหว่างออกกำลังกายจริงแบบเดียวกับ REC-2's MET calculation จึงไม่ควรอยู่ในรายการนี้ (ตัดออกแล้ว — ดู
  [detailed-design/03-planner-logging.md](detailed-design/03-planner-logging.md)'s ภาคผนวก Stack Mapping
  ที่แก้ให้ตรงกันในวันเดียวกันนี้) — จึง implement อัลกอริทึมที่เหลือ (TDEE/safety floor/MET) ที่ client
  โดยตรงเหมือนเดิม (React+Vite web client สำหรับ feature เหล่านี้ เพราะทั้งหมดอยู่ใน `apps/web` แล้วหลังตัด
  ขอบเขตมือถือ — ดูหัวข้อ 3) แล้วส่งผลลัพธ์ที่คำนวณแล้วไปบันทึกผ่าน **Express route** (แทนที่ Firebase Cloud
  Function เดิม) เพื่อให้ route นั้นทำหน้าที่ validate/บังคับกติกาธุรกิจเป็นเกราะป้องกันชั้นที่สองฝั่ง server
  ด้วย ไม่ใช่พึ่งพา client ฝ่ายเดียว — ส่วน streak recompute (PLN-4) เปลี่ยนจาก Firestore `onWrite` trigger
