...
- **อัปเดตล่าสุด:** 2026-09-26 (รอบ 15) — ONB-0/REQ-15: ปิดจุดที่ยังไม่ได้ระบุข้อ 10 ครึ่ง `POST /auth/login/email`
  — ตรวจกับระบบยืนยันตัวตนจริงแล้วว่ารหัสผ่านผิดกับอีเมลที่ไม่มีบัญชีได้ผลเดียวกัน (ไม่ leak account existence
  อย่างที่บันทึกไว้ 2026-09-07) และ Onboarding spec เพิ่ม decision "เข้าสู่ระบบไม่สำเร็จ (REQ-15)" — แก้แถว
  `POST /auth/login/email` ในหัวข้อ 3.1 และข้อ 10 ในหัวข้อ 4 ให้ตรง พร้อมเอาชื่อ stack ที่หลุดเข้ามาในเนื้อหาหลัก
  ของ 2 จุดนี้ออก (self-check กติกา conceptual) — merely-stale drift ไม่มี operation/payload ใหม่ ไม่แตะ
  `database-schema.md`
- **อัปเดตก่อนหน้า:** 2026-09-26 (รอบ 14) — INT-1/REQ-11: resolve จำนวนวัน log ขั้นต่ำก่อนพยากรณ์ = **7 วัน**
  (ยืนยันกับผู้ใช้งานแล้ว 2026-09-26 ใน `01-spec/20260823-04-smart-integrations.md`) — แก้เงื่อนไข `422` ของ
  `GET /insights/forecast` (หัวข้อ 3.7) เป็น "มี daily log สะสมน้อยกว่า 7 วัน" และปิดจุดที่ยังไม่ได้ระบุข้อ 3
  (หัวข้อ 4) — เป็น merely-stale drift ล้วน ไม่มีการเปลี่ยน operation/payload อื่น
- **อัปเดตก่อนหน้า:** 2026-09-25 (รอบ 13) — `feature-list-journey` เพิ่งเติม 2 กติกาธุรกิจใหม่เข้า REQ-18
  (`01-spec/20260823-04-smart-integrations.md`, ยืนยันจากผู้ใช้งาน 2026-09-25) และ implement จริงแล้ว: (1)
  **rate limit เมื่อกรอกรหัสจับคู่ผิดซ้ำ** — 5 ครั้งภายใน 15 นาทีต่อ client IP → ล็อกชั่วคราว (`429` +
  header `Retry-After`), client ที่ถูกล็อกถูกปฏิเสธก่อนแม้แต่จะตรวจสอบรหัส, redeem สำเร็จรีเซ็ตตัวนับทันที
  (2) **1 บัญชีมีรหัสจับคู่ที่ใช้งานได้ทีละ 1 รหัส** — ขอรหัสใหม่ (`POST /auth/pairing-codes`) ยกเลิกรหัสเก่า
  ของบัญชีเดียวกันทั้งหมดก่อนเสมอ และ (3) INT-3 เพิ่ม endpoint ใหม่ `GET /integrations/wearable/latest-session`
  + เปลี่ยนพฤติกรรม `POST /integrations/wearable/readings` ให้แก้ไข daily log ของ session ที่ปิดจบไปแล้วแบบ
  retroactive ด้วยส่วนต่าง (delta) — เปลี่ยนแปลงดังนี้: **§2 (Conventions)** เพิ่ม `429` เข้ารายการ status
  code — **§3.1** แก้แถว `POST /auth/pairing-codes` (เพิ่มพฤติกรรม invalidate รหัสเก่า) และแถว
  `POST /auth/pairing-codes/redeem` (เพิ่ม error case `429`/`Retry-After`) — **§3.2** แก้แถว `PUT
  /profile/goal` ทั้ง Request/Response: request เหลือแค่ประเภทเป้าหมาย+น้ำหนักเป้าหมาย (ค่า
  `dailyCalorieTargetKcal`/`dailyIntakeTargetKcal` ที่เคยให้ client ส่งมากลายเป็น legacy field ที่ระบบ
  **ไม่เชื่อถืออีกต่อไป** — server คำนวณเองจากน้ำหนักตัว/TDEE ที่บันทึกไว้แล้วเสมอ) response เปลี่ยนจาก "Goal
  Selection พร้อมค่าที่คำนวณ" เป็น **`204` ไม่มีเนื้อหา** (client อ่านค่าที่คำนวณจริงผ่าน `GET /profile` แยก
  ต่างหาก) เพิ่ม error `409` เมื่อยังไม่เคยทำ ONB-1 (ไม่มี weight/TDEE ให้คำนวณ) — **§3.8** เพิ่ม operation
  ใหม่ `GET /integrations/wearable/latest-session` (คืน session ล่าสุดใน 24 ชม.ที่ผ่านมา, `404` ถ้าไม่มี)
  และแก้แถว `POST /integrations/wearable/readings`: response เปลี่ยนจาก `204` เป็น `200 { appliedToLog,
  calculatedKcal }`, เพิ่มพฤติกรรมแก้ไข daily log ของ session ที่ปิดจบไปแล้วด้วยส่วนต่าง + recompute
  completion/streak, ระบุชัดว่า re-sync ซ้ำทำได้เสมอไม่ใช่ error — resolve จุดที่ยังไม่ได้ระบุข้อ 13 (rate
  limiting ของ pairing-codes) ในหัวข้อ 4 ด้วยรูปแบบ `~~...~~` **resolved** — **อัปเดต §6.3.1** (ภาคผนวก:
  Stack Mapping) 2 แถว `POST /auth/pairing-codes`/`.../redeem` ให้ระบุรายละเอียด implementation ใหม่ (auto-
  sync ตาม pattern ที่หัวข้อนี้เคยออกแบบเพิ่มเติมเองมาก่อน เพราะ `tech-stack.md` §6.3.1 ยังไม่เคยกล่าวถึง
  rate limit/invalidate-on-create เลย ควรนำกลับไปปรับให้ตรงกันในรอบ `tech-stack-builder` ถัดไป) — audit
  หัวข้อ 1, 3.3-3.7, 5 แล้วไม่พบ drift อื่นจากรอบนี้ (ดู log [2026-09-25](../../05-log/20260925-log.md))
- **อัปเดตก่อนหน้า:** 2026-09-07 (รอบ 12, follow-up) — split จุดที่ยังไม่ได้ระบุข้อ 10 (account enumeration
...
- **Status code ที่ใช้**: `200` สำเร็จ (อ่าน/อัปเดต), `201` สร้างสำเร็จ, `204` สำเร็จไม่มีเนื้อหาตอบกลับ,
  ...
  ต้องตามรูปแบบแต่ขัดกติกาธุรกิจ (เช่น พยายามตั้ง Cheat/Rest Day ย้อนหลังเกินวันนี้ตาม PLN-2), `429` ถูกจำกัด
  อัตราการเรียกชั่วคราว (เพิ่ม 2026-09-25 — ใช้เฉพาะ `POST /auth/pairing-codes/redeem` ในหัวข้อ 3.1 เมื่อ
  client เดียวกันกรอกรหัสผิด/หมดอายุ/ถูกใช้ไปแล้วซ้ำเกินเกณฑ์ที่กำหนด — มาพร้อม header `Retry-After` เชิง
  แนวคิดเสมอ)
...
| เข้าสู่ระบบด้วยอีเมล/รหัสผ่าน | `POST /auth/login/email` | ONB-0/REQ-15 | อีเมล, รหัสผ่าน | User Account ที่ตรงกัน + session ที่ถูกจดจำไว้ (session persistence) → พาไปหน้าที่เหมาะสม (ONB-1 ถ้ายังไม่เคยผ่าน หรือ Daily Dashboard ถ้าผ่านแล้ว) | `401` credential ไม่ถูกต้อง — **response เดียวกันทั้งรหัสผ่านผิดและอีเมลที่ไม่มีบัญชีในระบบ** ไม่เปิดเผยว่าอีเมลใดมีบัญชี (resolved 2026-09-26 — ตรวจกับระบบยืนยันตัวตนที่ใช้จริงแล้วว่าให้ผลเดียวกันทั้งสองกรณี และ [decision "เข้าสู่ระบบไม่สำเร็จ (REQ-15)" ใน Onboarding spec](../../01-requirements/01-spec/20260823-01-onboarding-personalization.md#ข้อสมมติฐานการตัดสินใจที่ยืนยันแล้ว) กำหนดให้ client แสดงข้อความเดียวกัน "อีเมลหรือรหัสผ่านไม่ถูกต้อง" แล้วอยู่หน้าเข้าสู่ระบบต่อ ดูจุดที่ยังไม่ได้ระบุข้อ 10) — ความล้มเหลวอื่น (รูปแบบอีเมลผิด, บัญชีถูกระงับ, ลองผิดหลายครั้งเกินไป, เชื่อมต่อไม่ได้) ก็ต้องแสดงเป็นข้อความภาษาไทยของสาเหตุนั้น ไม่ใช่ข้อความทางเทคนิคดิบ | NFR-04 |
...
| ตั้ง/แก้เป้าหมายหลัก | `PUT /profile/goal` | ONB-3/REQ-02 | ประเภทเป้าหมาย, น้ำหนักเป้าหมาย (บังคับเมื่อ "ลดน้ำหนัก") — **แก้ไข 2026-09-25 (ยืนยันกับผู้ใช้งานแล้ว): ไม่ต้องส่งค่าแคลอรี่ที่คำนวณเองมาอีกต่อไป** `dailyCalorieTargetKcal`/`dailyIntakeTargetKcal` ที่ไคลเอนต์รุ่นก่อนหน้าเคยคำนวณ/ส่งมาเองกลายเป็น **legacy field ที่ระบบไม่เชื่อถืออีกต่อไป** (ยอมรับได้เพื่อ backward compatibility แต่ถูกละเว้นเสมอ) — server เป็นผู้คำนวณทั้งสองค่านี้เองแบบ authoritative จากน้ำหนักตัว/TDEE ที่บันทึกไว้แล้วใน User Profile (ไม่ใช่แค่ตรวจสอบซ้ำค่าที่ client ส่งมาเหมือนที่เคยอธิบายไว้ก่อนหน้านี้) | **แก้ไข 2026-09-25: `204` ไม่มีเนื้อหาตอบกลับ** (ไม่ใช่ Goal Selection object แบบเดิมอีกต่อไป) — Goal Selection พร้อมทั้งสองเป้าหมายแคลอรี่ (`dailyCalorieTargetKcal`/`dailyIntakeTargetKcal`) และ `isSafetyFloorApplied` ถูกคำนวณ/บันทึกไว้ที่ server แล้วจริง แต่ client ต้องเรียก `GET /profile` แยกต่างหากเพื่ออ่านค่าที่คำนวณจริง (การแสดงตัวอย่างระหว่าง onboarding ยังใช้ค่าที่ client คำนวณเองไว้ก่อนหน้าเพื่อความไว NFR-01/03 — ไม่ใช่ค่าจาก response นี้) — ใช้กับเป้าหมาย intake เท่านั้นสำหรับ safety floor เพราะเป้าหมายการออกกำลังกายไม่มี safety floor | `400` ขาดน้ำหนักเป้าหมายตอนเลือก "ลดน้ำหนัก", **`409` ใหม่ 2026-09-25** ถ้ายังไม่เคยทำ ONB-1 ให้เสร็จ (ไม่มีน้ำหนักตัว/TDEE ให้คำนวณ — ไม่ fallback ไปใช้ legacy field จาก client แทน เพราะการเชื่อค่า client คือสิ่งที่กำลังแก้) | NFR-01 (คำนวณ client-side สำหรับ preview ระหว่าง onboarding) |
...
| Operation | Verb + Path | Feature/REQ | Request | Response | Error/Edge Case | NFR |
|---|---|---|---|---|---|---|
| ดูพยากรณ์เป้าหมายน้ำหนัก | `GET /insights/forecast` | INT-1/REQ-11 | — | วันที่คาดว่าจะถึงเป้าหมาย, อัตราขาดดุลเฉลี่ย | `422` ถ้ายังไม่มีน้ำหนักเป้าหมาย (ต้องกรอกที่ ONB-3 ก่อน), `422` ถ้ามี daily log สะสม**น้อยกว่า 7 วัน** (client แสดง empty state "ประวัติยังไม่พอสำหรับพยากรณ์" แทนวันที่พยากรณ์ ไม่แสดงวันที่ประมาณการใดๆ — ยืนยันกับผู้ใช้งานแล้ว 2026-09-26 ตาม `01-spec/20260823-04-smart-integrations.md` § ข้อสมมติฐาน/การตัดสินใจที่ยืนยันแล้ว INT-1/REQ-11) | — |
...
| ดู Workout Session ล่าสุดที่ยังซิงค์ wearable ได้ (เพิ่ม 2026-09-25) | `GET /integrations/wearable/latest-session` | INT-3/REQ-13 (companion app ไม่มีหน้าจอบันทึกการออกกำลังกายของตัวเอง — การจบเซสชันทำที่เว็บ Planner เสมอ ตาม `01-spec/20260823-04-smart-integrations.md` § REQ-13 decision 2026-09-25) | — | Workout Session ล่าสุดของผู้ใช้ที่เริ่ม**ภายใน 24 ชั่วโมงที่ผ่านมา** (id, เวลาเริ่ม, เวลาที่ใช้จริงถ้าจบแล้ว, สถานะ, มี Wearable Reading อยู่แล้วหรือยัง) — ให้ไคลเอนต์ตัดสินใจต่อว่าจะซิงค์ได้เลยหรือต้องรอ | `404` ถ้าไม่มี session ใดเริ่มภายใน 24 ชม.ที่ผ่านมาเลย — เมื่อพบ session แต่สถานะยัง "กำลังดำเนินอยู่" (ยังไม่จบที่เว็บ) ไคลเอนต์ต้องแจ้งผู้ใช้ให้ไปจบที่เว็บก่อน (ไม่ใช่ error ของ operation นี้ — response ยังคืน `200` ปกติพร้อมสถานะนั้น) | NFR-04 (ข้อมูลสุขภาพ) |
| ส่งค่าแคลอรี่จาก wearable | `POST /integrations/wearable/readings` | INT-3/REQ-13 | sessionId, ค่าแคลอรี่จาก wearable | **แก้ไข 2026-09-25 (ยืนยันกับผู้ใช้งานแล้ว): `200 { appliedToLog, calculatedKcal }`** (เดิม `204`) — Wearable Reading ที่บันทึก → ใช้แทนค่าประมาณ MET ใน `POST /workouts/sessions/{sessionId}/complete` ถ้ามาถึงก่อน session นั้นปิดจบ **หรือถ้า session นั้นปิดจบไปแล้ว ให้แทนที่ค่า MET เดิมด้วยค่าจาก wearable ทันที (retroactive) แล้วแก้ไข Daily Log ของวันที่ session นั้นบันทึกไว้ (ไม่ใช่ "วันนี้" ของตอนซิงค์เสมอไป เพราะอาจซิงค์ข้ามเที่ยงคืน) ด้วยส่วนต่าง (delta) ระหว่างค่าใหม่กับค่าเดิมที่ session นั้นเคยมีส่วนสมทบไว้แล้วเท่านั้น** (ไม่ใช่ค่าเต็มจำนวน — ป้องกัน re-sync ซ้ำนับแคลอรี่ซ้ำเมื่อวันนั้นมีหลาย session) แล้ว recompute สถานะ "ครบเป้าหมาย" (PLN-3) และ streak (PLN-4) ของวันนั้นใหม่ทันที — `appliedToLog` = true เมื่อ session ปิดจบไปแล้วและมีการแก้ไข Daily Log จริง (false เมื่อ session ยังไม่จบ — ค่าจะถูกใช้ตอน complete แทน) | `404` sessionId ไม่พบ, ถ้าไม่มีข้อมูล wearable ระบบใช้ค่าประมาณ MET ตามเดิม (ไม่ใช่ error), **การ re-sync ซ้ำสำหรับ session เดิมทำได้เสมอ ไม่ถือเป็น error** (เขียนทับ Wearable Reading เดิมและคำนวณ delta ซ้ำทุกครั้ง — ยืนยันกับผู้ใช้งานแล้ว 2026-09-25) | NFR-04, NFR-07, NFR-12 (referential existence validation ของ sessionId) |
...
3. ~~**INT-1**: จำนวนวัน log ขั้นต่ำก่อนพยากรณ์ได้ยังไม่ระบุ — `GET /insights/forecast`'s เงื่อนไข `422`
   "log สะสมไม่พอ" ยังไม่มีตัวเลขที่แน่นอน~~ — **resolved 2026-09-26 (ยืนยันกับผู้ใช้งานแล้ว)**: ต้องมี
   daily log สะสม**อย่างน้อย 7 วัน**ก่อนพยากรณ์ได้ (หนึ่งรอบ Weekly Planner จันทร์–อาทิตย์เต็ม) — น้อยกว่า
   7 วัน → `422` และ client แสดง empty state ตาม
   [`01-spec/20260823-04-smart-integrations.md` § ข้อสมมติฐาน/การตัดสินใจที่ยืนยันแล้ว (INT-1/REQ-11)](../../01-requirements/01-spec/20260823-04-smart-integrations.md)
   — แก้เงื่อนไข `422` ของ `GET /insights/forecast` ในหัวข้อ 3.7 ให้ตรงแล้ว ไม่ใช่ open point อีกต่อไป
...
5. **Rate limiting / pagination**: ยังไม่มี requirement ใดระบุถึงเรื่อง pagination — `GET /logs` ที่คืน
   ประวัติทั้งหมดอาจต้องมี pagination เมื่อข้อมูลมากขึ้น แต่ยังไม่ได้ตัดสินใจรูปแบบ (**ส่วน rate limiting ของ
   `POST /auth/pairing-codes/redeem` โดยเฉพาะ resolve แล้ว 2026-09-25 — ดูข้อ 13 ด้านล่างและหัวข้อ 3.1 —
   จุดนี้ยังเปิดอยู่เฉพาะ pagination กับ rate limit ของ endpoint อื่นที่เหลือทั้งหมดเท่านั้น**)
...
10. ~~**ONB-0**: พฤติกรรมของ `POST /auth/login/email` และ `POST /auth/forgot-password` เมื่ออีเมลไม่มีอยู่
    ...
    ไม่ได้ตัดสินใจ~~ — **resolved ทั้งหมด** (ทั้งสอง operation ไม่เปิดเผยว่าอีเมลใดมีบัญชี):
    - **`POST /auth/forgot-password`** (resolved 2026-09-07): เมื่ออีเมลไม่มีอยู่ในระบบเลย ได้ response `202`
      เดียวกันกับกรณีอีเมลมีอยู่จริงเสมอ — เป็นการตัดสินใจ anti-enumeration ที่ตั้งใจตามแนวทางที่ OWASP แนะนำ
      สำหรับ password-reset flow ไม่ใช่ byproduct
    - **`POST /auth/login/email`** (resolved 2026-09-26): เดิมบันทึกไว้ (2026-09-07) ว่าระบบยืนยันตัวตนคืนผล
      แยกกันระหว่างอีเมลที่ไม่มีบัญชีกับรหัสผ่านผิด จึงยัง leak account existence — ตรวจกับระบบยืนยันตัวตนที่ใช้
      จริงเมื่อ 2026-09-26 แล้วพบว่า**ทั้งสองกรณีได้ผลเดียวกัน** (เปิดการป้องกัน account enumeration ไว้แล้ว) ข้อ
      สรุปเดิมจึงไม่จริงอีกต่อไป และไม่ต้องย้าย login ไปผ่าน server ของทีม — ฝั่ง client ก็แสดงข้อความเดียวกันทั้ง
      สองกรณีตาม [decision "เข้าสู่ระบบไม่สำเร็จ (REQ-15)" ใน Onboarding spec](../../01-requirements/01-spec/20260823-01-onboarding-personalization.md#ข้อสมมติฐานการตัดสินใจที่ยืนยันแล้ว)
    - ข้อควรระวังที่เหลือ (ไม่ใช่ open point ของเอกสารนี้): พฤติกรรมนี้ขึ้นกับการตั้งค่าของระบบยืนยันตัวตน ถ้ามีคนปิด
      การป้องกันนี้ทีหลัง `401` จะกลับมาแยกกรณีได้อีก — รายละเอียดการตั้งค่าเป็นของ
...
12. ~~**Pairing-code mechanism**: ยังไม่มี REQ number formal สำหรับกลไกนี้~~ — **resolved 2026-08-30
    ...
    **REQ-18** แล้ว (แทนที่การอ้างอิงแบบ implicit precondition ของ REQ-12/REQ-13 เดิม — ดู
    ...
    point อีกต่อไป
13. ~~**`POST /auth/pairing-codes`**: ยังไม่ระบุว่าอนุญาตให้มีรหัสที่ยังไม่หมดอายุ/ยังไม่ถูกใช้มากกว่า 1
    รหัสต่อ User Account เดียวกันพร้อมกันหรือไม่ ... และควรมี rate limit ต่อการกดขอรหัสซ้ำถี่ๆ หรือไม่~~ —
    **resolved 2026-09-25 (ยืนยันกับผู้ใช้งานแล้ว)**: **ไม่อนุญาต** — `POST /auth/pairing-codes` ยกเลิก
    รหัสเก่าของบัญชีเดียวกันทั้งหมดก่อนออกรหัสใหม่เสมอ (1 บัญชีมีรหัสที่ใช้งานได้ทีละ 1 รหัส) และ
    `POST /auth/pairing-codes/redeem` มี **rate limit ต่อ client IP** (5 ครั้งผิดภายใน 15 นาที → `429` +
    `Retry-After`) ป้องกัน brute-force ของรหัส 6 หลัก — ดูรายละเอียดเต็มในหัวข้อ 3.1 — ไม่ใช่ open point
    อีกต่อไป
...
| `POST /auth/pairing-codes` (ใหม่ 2026-08-30, เพิ่ม invalidate 2026-09-25) | **Express route** `POST /api/pairing/create-code` (`apps/web/server/routes/pairing/index.ts`, ต้องผ่าน `authenticate` middleware ก่อนเสมอ) — **ก่อนสร้างรหัสใหม่**: query top-level collection `pairingCodes` ที่ `uid == req.userId` แล้ว batch-delete ทุกแถวที่พบ (`invalidateExistingCodesFor()`, เพิ่ม 2026-09-25 — enforce "1 บัญชีมีรหัสที่ใช้งานได้ทีละ 1 รหัส") จากนั้นสร้าง document ใหม่ที่ `pairingCodes/{code}` ด้วย `.create()` (ไม่ใช่ `.set()` — กันชนกับรหัสที่ยังไม่หมดอายุของผู้ใช้อื่นถ้าเกิด random collision, retry สูงสุด 5 ครั้งด้วยรหัสใหม่ถ้าชน) เก็บ `uid`/`createdAt`/`expiresAt` (TTL 5 นาที) แล้วคืนรหัส + เวลาหมดอายุ |
| `POST /auth/pairing-codes/redeem` (ใหม่ 2026-08-30, เพิ่ม rate limit 2026-09-25) | **Express route** `POST /api/pairing/redeem` (**ข้อยกเว้นเดียว** ไม่มี `authenticate` middleware) — **ก่อนอ่าน `pairingCodes/{code}` เลย**: ตรวจ lock ของ client IP นี้จาก top-level document `pairingRedeemAttempts/{sha256(ip)}` (pre-check นอก transaction เพื่อลดโหลด + re-check ซ้ำในธุรกรรมเดียวกับการอ่าน/ลบรหัสกันการ race) ถ้าถูกล็อกอยู่คืน `429` พร้อม header `Retry-After` ทันที (ไม่แตะ `pairingCodes` เลย) — ตรรกะ lock/increment/reset เป็นฟังก์ชันล้วนแยกไฟล์ `apps/web/server/domain/pairingRateLimit.ts` (`isLocked`/`recordFailedAttempt`/`retryAfterSeconds`, MAX_FAILED_ATTEMPTS=5, WINDOW_MS=15 นาที) ไม่แตะ Firestore/Express โดยตรง — ถ้าไม่ถูกล็อก: อ่าน `pairingCodes/{code}`, ถ้าไม่พบหรือหมดอายุ (`expiresAt < now`) บันทึกความล้มเหลวลง `pairingRedeemAttempts` แล้วคืน `410 Gone`, ถ้าสำเร็จ `delete()` ทั้ง `pairingCodes/{code}` (single-use ผ่านการลบ) และ `pairingRedeemAttempts/{sha256(ip)}` (รีเซ็ตตัวนับ) ในธุรกรรมเดียวกัน แล้วเรียก `auth.createCustomToken(uid)` คืน custom token — status code `410` ครอบคลุมทั้ง "ไม่พบ"/"หมดอายุ"/"ถูกใช้ไปแล้ว" เป็นเงื่อนไขเดียว เพราะเมื่อ document ถูกลบไปแล้ว 3 กรณีนี้แยกไม่ออกจากกันอีกต่อไป (แก้ error case ในหัวข้อ 3.1 ให้ตรงกับพฤติกรรมนี้แล้ว 2026-08-30 รอบ 5) — client IP hash ด้วย SHA-256 เท่านั้น **ไม่เก็บ IP ดิบ** |
