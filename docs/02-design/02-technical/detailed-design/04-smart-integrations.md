...
- **อัปเดตล่าสุด:** 2026-09-26 — แก้ **INT-1** เท่านั้น ตาม decision ที่ผู้ใช้ยืนยันแล้ว 2026-09-26 ใน
  [`01-spec/20260823-04-smart-integrations.md` § ข้อสมมติฐาน/การตัดสินใจที่ยืนยันแล้ว](../../../01-requirements/01-spec/20260823-04-smart-integrations.md#ข้อสมมติฐานการตัดสินใจที่ยืนยันแล้ว):
  **จำนวนวัน log ขั้นต่ำก่อนพยากรณ์ได้ = 7 วัน** (มีอย่างน้อย 7 วันก็พยากรณ์ได้ — ครบ 7 วันพอดีถือว่าพอ) —
  เอกสารล้าหลัง ไม่ใช่ข้อขัดแย้ง แก้โดยตรง: (1) `alt` block ใน sequence diagram ของ INT-1 จาก "จำนวนขั้นต่ำยัง
  ไม่ระบุ" เป็น "น้อยกว่า 7 วัน" (2) step 2 ของอัลกอริทึม "คำนวณวันพยากรณ์ถึงเป้าหมายน้ำหนัก" ระบุเกณฑ์ 7 วัน
  ชัดเจน (น้อยกว่า 7 วัน → ข้อมูลสะสมไม่พอ) (3) จุดที่ยังไม่ได้ระบุ #1 เปลี่ยนเป็น resolve แล้ว พร้อมลิงก์ไปยัง
  decision ต้นทาง (4) หมายเหตุในบรรทัด "อัปเดตก่อนหน้า 2026-08-31" ที่เคยอ้างค่า placeholder 3 วันว่ายังไม่
  resolve — เติมหมายเหตุว่าถูกแทนที่แล้ว — โค้ดจริงสอดคล้องแล้ว (ค่าคงที่ขั้นต่ำ = 7 ใน
  `apps/web/server/domain/weightForecast.ts`) — ภาคผนวก Stack Mapping ไม่ได้อ้างถึงค่า 3 วันจึงไม่ต้องแก้ —
  ไม่แตะ INT-0/2/3 หรือ State Diagram
- **อัปเดตก่อนหน้า:** 2026-09-25 — `feature-list-journey`/`api-db-spec-writer` เพิ่งเติม 2 กติกาธุรกิจใหม่เข้า
...
- **อัปเดตก่อนหน้า:** 2026-08-31 — audit เทียบกับโค้ดจริงที่เพิ่ง ship (commit `b463436`,
  ...
  **ค่าเฉลี่ยของแคลอรี่ที่เผาผลาญจริงต่อวัน (accumulatedKcal ต่อวัน)** ตรงๆ ไม่ใช่ผลต่างจากเป้าหมาย — เพราะแอปนี้
  ...
  ล้าหลังไปแล้วเช่นกัน (implement จริงและ ship แล้วเต็มรูปแบบ) — ณ ขณะนั้นตรวจค่า placeholder 3 วันในโค้ด
  แล้วยืนยันว่า**ยังไม่ใช่ decision ที่ resolve เป็นทางการ** — คงจุดที่ยังไม่ได้ระบุ #1 (จำนวนวัน log
  ขั้นต่ำ) ไว้ตอนนั้น (**ถูกแทนที่แล้ว 2026-09-26** — resolve เป็นทางการที่ 7 วัน ดู "อัปเดตล่าสุด" ด้านบน)
  — ไม่พบ drift อื่นใน INT-0/2/3/State Diagram (ดู [log 2026-08-31](../../../05-log/20260831-log.md))
...
sequenceDiagram
    ...
    alt ไม่มีน้ำหนักเป้าหมาย
        ...
        alt log สะสมไม่พอ (น้อยกว่า 7 วัน — resolve แล้ว 2026-09-26)
            IF-->>U: 422 Unprocessable (ข้อมูลสะสมไม่พอ — ต้องสะสมให้ครบอย่างน้อย 7 วัน)
        else log สะสมเพียงพอ (อย่างน้อย 7 วัน)
...
2. นับจำนวนวันที่มีประวัติ `daily_log` สะสม → ถ้า**น้อยกว่า 7 วัน** ถือว่าข้อมูลสะสมไม่พอ
   (not_enough_history) คืน error (`422`) — มีครบ 7 วันพอดีถือว่าพอ พยากรณ์ต่อได้ (เกณฑ์ 7 วัน resolve แล้ว
   2026-09-26 ใน [Requirement § ข้อสมมติฐาน/การตัดสินใจที่ยืนยันแล้ว](../../../01-requirements/01-spec/20260823-04-smart-integrations.md#ข้อสมมติฐานการตัดสินใจที่ยืนยันแล้ว))
...
1. ~~**INT-1**: จำนวนวัน log ขั้นต่ำก่อนพยากรณ์ได้ยังไม่ระบุเป็นทางการ~~ — **resolve แล้ว 2026-09-26: 7 วัน**
   (มีอย่างน้อย 7 วันก็พยากรณ์ได้ น้อยกว่านั้นคืน error ข้อมูลสะสมไม่พอ) ดู
   [Requirement § ข้อสมมติฐาน/การตัดสินใจที่ยืนยันแล้ว](../../../01-requirements/01-spec/20260823-04-smart-integrations.md#ข้อสมมติฐานการตัดสินใจที่ยืนยันแล้ว)
   — สะท้อนแล้วใน sequence diagram และ step 2 ของอัลกอริทึม INT-1 ด้านบน
