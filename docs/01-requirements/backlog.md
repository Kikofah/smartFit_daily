...
- **คำอธิบาย**: ผู้ใช้สมัครสมาชิก (Sign-up) ได้ 3 วิธี — email/password, Google OAuth, หรือ Sign in with
  ...
  login ซ้ำจนกว่าจะออกจากระบบเองหรือ session หมดอายุ (REQ-15) ถ้าเข้าสู่ระบบไม่สำเร็จ ผู้ใช้อยู่หน้าเข้าสู่ระบบ
  ต่อและเห็นข้อความ error ภาษาไทย โดยรหัสผ่านผิดกับอีเมลที่ไม่มีบัญชีใช้ข้อความเดียวกัน "อีเมลหรือรหัสผ่านไม่
  ถูกต้อง" (REQ-15, ยืนยัน 2026-09-26 ดู
  [decision ใน Onboarding spec](01-spec/20260823-01-onboarding-personalization.md#ข้อสมมติฐานการตัดสินใจที่ยืนยันแล้ว))
  ผู้ใช้ที่สมัครด้วย email/password และลืม
...
- **คำอธิบาย**: ผู้ใช้เลือกเป้าหมายหลัก (ลดน้ำหนัก / กระชับสัดส่วน / เพิ่มความอึด) ระบบแปลงเป็น
  **ตัวเลขแคลอรี่ 2 ค่าแยกกัน** ตาม[decision ที่ยืนยันแล้วเมื่อ 2026-08-31 ใน Onboarding
  ...
  ใช้พยากรณ์วันถึงเป้าหมาย — **เพิ่มเติม 2026-09-25 ยืนยันจากผู้ใช้งานแล้ว**: ทั้งสองค่าถูกเก็บเป็น**ค่าที่
  แม่นยำ ไม่ปัดเศษ** (เช่น 337.5 ไม่ใช่ 338 — ปัดเศษเป็นเรื่องของการแสดงผลเท่านั้น), safety floor มีผลเฉพาะ
  เมื่อค่าดิบ**ต่ำกว่า** 1,200 kcal อย่างเคร่งครัด (พอดี 1,200 ไม่ถูกปรับ), และ server เป็นผู้คำนวณค่าทั้งสอง
  แบบเป็นทางการ (authoritative) เองจากน้ำหนัก/TDEE ที่บันทึกไว้ ไม่เชื่อค่าตัวเลขจาก client โดยตรง — ดู
  [decision เต็มใน Onboarding spec](01-spec/20260823-01-onboarding-personalization.md#ข้อสมมติฐานการตัดสินใจที่ยืนยันแล้ว)
...
- **คำอธิบาย**: ระบบดึงเป้าหมายแคลอรี่ที่เหลือของวันนี้จาก ONB-3 (ปรับตาม Cheat/Rest Day จาก PLN-2 ถ้ามี)
  แล้วค้นหาวิดีโอจาก YouTube ด้วย filter อุปกรณ์จาก ONB-2 **ครั้งเดียว** (single-pass ไม่มีตัวเลข tolerance
  และไม่มีการขยายเกณฑ์ค้นหาซ้ำ) จากนั้นส่งเข้าขั้นตอนจับคู่/ประเมินด้วย AI ครั้งเดียวเพื่อเลือกวิดีโอที่
  เหมาะสมที่สุดแบบ best-effort — ถ้ารอบเดียวนี้ไม่พบวิดีโอที่ใช้ได้เลย ระบบแจ้งผู้ใช้ทันทีโดยไม่ลองใหม่
  ผลลัพธ์จาก feature นี้จะถูกส่งต่อให้ REC-2 คำนวณแคลอรี่จริงหลังออกกำลังกาย (รายละเอียดอัลกอริทึมดู
...
- **คำอธิบาย**: ปุ่ม "เปลี่ยนวิดีโอ"/"ลองวิดีโออื่น" เก็บค่าแคลอรี่เป้าหมายของวันไว้คงเดิม (ไม่ดึงเป้าหมายใหม่)
  แล้วค้นหาวิดีโอใหม่จาก YouTube **ครั้งเดียว** ด้วย filter อุปกรณ์ + แคลอรี่เป้าหมายเดียวกันจาก REC-1 โดยไม่
  รวมวิดีโอปัจจุบันและวิดีโอที่เคยถูกปฏิเสธไปก่อนหน้านี้ทั้งหมด — ใช้อัลกอริทึมค้นหา+จับคู่ด้วย AI แบบ
  single-pass เดียวกับ REC-1 (ไม่มีการขยายเกณฑ์ค้นหาซ้ำ) ถ้าไม่พบวิดีโอที่ใช้ได้ ระบบแจ้งผู้ใช้ทันที
...
- **คำอธิบาย**: ผู้ใช้ที่ล็อกอินอยู่บนเว็บแอปแล้ว ([ONB-0](#onb-0--สมัครสมาชิก--เข้าสู่ระบบ--ลืมรหัสผ่าน--ออกจากระบบ-authentication))
  ...
  **5 นาที** และเป็น **single-use** จากนั้นผู้ใช้กรอกรหัสนี้บน companion mobile app เพื่อแลกเป็น session
  ...
  และ [INT-3](#int-3--ซิงค์ข้อมูล-wearable) ด้านล่าง — **เพิ่มเติม 2026-09-25 ยืนยันจากผู้ใช้งานแล้ว**:
  กรอกรหัสผิด **5 ครั้งภายใน 15 นาที ต่อ client IP** → ถูกล็อกชั่วคราว (`429` + `Retry-After`) จนกว่าหน้าต่าง
  เวลาจะหมด (client ที่ถูกล็อกถูกปฏิเสธก่อนแม้แต่จะตรวจรหัส, redeem สำเร็จรีเซ็ตตัวนับทันที) และ **1 บัญชี
  มีรหัสจับคู่ที่ใช้งานได้ทีละ 1 รหัสเท่านั้น** (ขอรหัสใหม่ = ยกเลิกรหัสเก่าที่ยังไม่หมดอายุ/ยังไม่ถูกใช้ของ
  บัญชีเดียวกันทั้งหมด) ดูรายละเอียดเต็มที่
  [decision ของ Smart Integrations spec](01-spec/20260823-04-smart-integrations.md#ข้อสมมติฐานการตัดสินใจที่ยืนยันแล้ว)
...
- **คำอธิบาย**: เชื่อมต่อ Apple Watch/Fitbit/Garmin ผ่าน Apple Health/Google Health Connect เพื่อดึง
  ...
  **[INT-0](#int-0--ยืนยันตัวตนก่อนจับคู่อุปกรณ์ผ่านรหัสจับคู่-pairing-code)** (REQ-18) เพื่อยืนยันตัวตน
  ...
  ซ้ำที่นี่อีกแล้วตั้งแต่ 2026-08-30) — **เพิ่มเติม 2026-09-25 ยืนยันจากผู้ใช้งานแล้ว**: เนื่องจาก companion
  app ไม่มีหน้าจอบันทึกการออกกำลังกายของตัวเองเลย (ทำที่หน้าเว็บ Planner ทั้งหมด — PLN-3) การ sync จึงเป็น
  แบบผู้ใช้กดเอง: จบ session ที่เว็บก่อน แล้วกดปุ่ม "ซิงค์แคลอรี่จากการออกกำลังกายครั้งล่าสุด" บนมือถือ
  แอปจะดึง session ล่าสุดใน 24 ชม.ที่ผ่านมาจาก server — ถ้า session ยัง "กำลังดำเนินอยู่" จะแจ้งให้ไปจบที่
  เว็บก่อน ถ้า "จบแล้ว" จะอ่านค่า Active Calories เฉพาะช่วงเวลาของ session นั้นแล้วส่งไปแทนที่ค่าประมาณ MET
  เดิม พร้อมแก้ไข daily log ของวันนั้นด้วยส่วนต่าง (delta) เพื่อไม่ให้ re-sync นับซ้ำ แล้วคำนวณสถานะครบ
  เป้าหมาย (PLN-3) และ streak (PLN-4) ใหม่ทันที ดูรายละเอียดเต็มที่
  [decision ของ Smart Integrations spec](01-spec/20260823-04-smart-integrations.md#ข้อสมมติฐานการตัดสินใจที่ยืนยันแล้ว)
