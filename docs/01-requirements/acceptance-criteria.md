...
- **Then**: ระบบบันทึกทั้งเป้าหมายแคลอรี่รายวัน (TDEE − 500 kcal/วัน ปรับตาม safety floor ถ้าจำเป็น) และ
  ...
  ทันที
- หมายเหตุ: ยังไม่มี prototype screen ใน `v1` ที่ implement ช่องกรอกน้ำหนักเป้าหมายนี้จริง (ดู gap ใน
  รายงานผลของ `test-suite-builder`)
...
- หมายเหตุ: ยังไม่มี prototype screen ใน `v1` ที่ implement ช่องกรอกน้ำหนักเป้าหมายนี้จริง (ดู gap ใน
  รายงานผลของ `test-suite-builder`) — ช่องทางที่ผู้ใช้จะกลับมากรอกภายหลังก็ยังไม่ถูกระบุ (ดู
