...
> ตรงกับ TC ด้านล่างทุกกรณี นอกจากนี้โค้ดยัง hardcode **`MIN_LOG_DAYS_FOR_FORECAST = 3`** (ต้องมี log
> อย่างน้อย 3 วันก่อนพยากรณ์ได้) ทั้งที่ `01-spec/20260823-04-smart-integrations.md` § จุดที่ยังไม่ได้ระบุ
> ยังคงระบุว่าจำนวนวันขั้นต่ำ **ยังไม่ resolve อย่างเป็นทางการ** (โค้ดเองก็ comment กำกับไว้ตรงๆ ว่า "NOT
> resolved... picked pragmatically") — เป็น **spec-vs-code mismatch ที่ยังไม่ resolve** (ดู Risk R4 ใน
> test-plan.md) จึงไม่ปักหมุดเลข 3 ลงในเนื้อหา AC/TC ที่นี่โดยตรง แต่ปรับ Pre-condition ด้านล่างให้ระบุ
> "อย่างน้อย 3 วัน" อย่างโปร่งใสว่ามาจากการตรวจโค้ด ไม่ใช่ resolve แล้วอย่างเป็นทางการ — **แนะนำให้ยืนยัน
> ตัวเลขนี้ผ่าน `feature-list-journey` ก่อน lock เป็นค่าทางการ**
...
- **Pre-condition**: ผู้ใช้ (persona ด้านบน) มีเป้าหมายน้ำหนัก 75.0 กก. บันทึกไว้ (จาก ONB-3) และมีประวัติ
  log แคลอรี่จริงสะสมจาก PLN-3 **อย่างน้อย 3 วัน** (ยืนยันจากโค้ดจริง `MIN_LOG_DAYS_FOR_FORECAST` — ดู
  หมายเหตุด้านบน) โดยอัตราขาดดุลเฉลี่ยที่คำนวณได้จาก log ไม่เป็น 0 และสอดคล้องกับทิศทางเป้าหมาย (ลดน้ำหนัก)
- **Test Steps**:
...
- **Test Data**: น้ำหนักปัจจุบัน 80.0 กก. เป้าหมาย 75.0 กก. ประวัติ log แคลอรี่เผาผลาญจริงอย่างน้อย 3 วัน
  เฉลี่ย = 500 kcal/วัน (ค่าอิสระ เลือกให้หารลงตัวพอดีกับตัวอย่างนี้ — **ไม่ผูกกับสูตร `dailyIntakeTargetKcal`
...
### TC-INT-1-002 — ประวัติ log ไม่เพียงพอ (น้อยกว่าขั้นต่ำที่โค้ด implement) แสดง empty state แทนวันที่พยากรณ์

- **Pre-condition**: ผู้ใช้เพิ่งผ่าน onboarding และตั้งเป้าหมายน้ำหนัก 75.0 กก. แต่ยังมีประวัติ log แคลอรี่
  จริงสะสมน้อยมาก (เช่น เพิ่งเริ่มใช้แอป 1 วัน — น้อยกว่า `MIN_LOG_DAYS_FOR_FORECAST` ของโค้ดจริง = 3 วัน
  ไม่ว่าตัวเลขที่ resolve อย่างเป็นทางการในอนาคตจะเป็นเท่าไหร่ก็ตาม เพราะ 1 < 3 อย่างชัดเจน)
- **Test Steps**:
...
- **Expected Result**: ระบบไม่แสดงวันที่คาดการณ์ใด ๆ แต่แสดง empty state ที่มีข้อความทั่วไปว่ายังพยากรณ์
  ...
  **ทดสอบเฉพาะว่า empty state ปรากฏและไม่มีวันที่ปลอมแสดงขึ้นมา ไม่ทดสอบจำนวนวันขั้นต่ำที่แน่นอนตัวเลขทางการ**
  เพราะเกณฑ์ตัวเลขยังเป็น Open Question ที่ยังไม่ resolve อย่างเป็นทางการ (ดูหมายเหตุท้ายไฟล์และหมายเหตุด้านบน)
