...
> **แก้ไข 2026-09-26**: Persona นี้เดิมใช้สูตร "เป้าหมายแคลอรี่รายวัน = TDEE − 500" เพียงค่าเดียว ซึ่งเป็น
> โมเดลเก่าก่อนที่ ONB-3/REQ-02 จะถูกแยกเป็น **2 ค่า** เมื่อ 2026-08-31 (ดู
...
> — ยืนยันจากโค้ดจริงแล้วว่า `dailyCalorieTargetKcal` (น้ำหนักตัว × ค่าคงที่ต่อเป้าหมาย) คือค่าเดียวที่
> REC-1 (`content-recommendation/index.ts`), PLN-3 (`exertion-calorie/index.ts`,
> `integration-gateway/index.ts`) และ INT-1 (เป็นเป้าหมายที่ทำให้เกิดยอดสะสมที่ INT-1 เอาไปเฉลี่ยต่อ) ใช้จริง
> ส่วน `dailyIntakeTargetKcal` (TDEE ± ค่าคงที่ต่อเป้าหมาย) ยังไม่มีฟีเจอร์ใดใน Epic 4 ใช้คำนวณจริง แก้ persona
> ด้านล่างให้ระบุค่าทั้งสองแยกกันชัดเจน ตรงกับ `test-cases/01-onboarding-personalization.md`

ผู้ใช้ประจำที่ผ่าน onboarding แล้ว: อายุ 30 ปี เพศชาย ส่วนสูง 170 ซม. น้ำหนักปัจจุบัน (ก่อนซิงค์) 80.0 กก.
Activity Factor 1.55 (ตัวอย่างระดับกิจกรรมปานกลาง) → TDEE = 10×80 + 6.25×170 − 5×30 + 5 = 1,717.5 BMR
× 1.55 = **2,662 kcal/วัน** (ใช้เป็น baseline ของ ONB-1 เท่านั้น) เป้าหมายหลัก = ลดน้ำหนัก →
**`dailyCalorieTargetKcal`** (เป้าหมายเผาผลาญจากการออกกำลังกาย, ไม่มี safety floor — ค่าที่ REC-1/PLN-3/INT-1
ใช้จริงวันนี้) = น้ำหนักตัว × 4.5 = 80.0 × 4.5 = **360.0 kcal/วัน** — **`dailyIntakeTargetKcal`** (เป้าหมาย
ที่ควรได้รับต่อวัน, สำรองไว้สำหรับฟีเจอร์บันทึกอาหารในอนาคต ยังไม่มีฟีเจอร์ใดใน Epic 4 ใช้จริง) = TDEE − 500 =
...
> **หมายเหตุ (เพิ่ม 2026-09-26)**: ยืนยันจากโค้ดจริง `apps/web/server/domain/weightForecast.ts` ว่า
> "อัตราขาดดุลเฉลี่ย" ที่ INT-1 ใช้จริงคือ**ค่าเฉลี่ยของ `accumulatedKcal`** (แคลอรี่ที่เผาผลาญจากการออก
> กำลังกายจริงต่อวัน จาก PLN-3) ไม่ใช่ TDEE ลบด้วยแคลอรี่ที่กิน (ยังไม่มีฟีเจอร์บันทึกอาหาร) — ตรงกับ
> ทิศทางของ `dailyCalorieTargetKcal` (เผาผลาญ) ไม่ใช่ `dailyIntakeTargetKcal` (TDEE ± ค่าคงที่) ค่าคงที่
> **7,700 kcal ≈ 1 กก.** ที่ใช้แปลงตรงกับที่ resolve ไว้ใน ONB-3/REQ-02 (`KCAL_PER_KG` ในโค้ดเดียวกัน) —
> ตรงกับ TC ด้านล่างทุกกรณี
>
> **อัปเดต 2026-09-26 — จำนวนวัน log ขั้นต่ำ resolve แล้ว = 7 วัน**: เดิมหมายเหตุนี้ flag ว่าโค้ด hardcode
> `MIN_LOG_DAYS_FOR_FORECAST = 3` ขณะที่ spec ยังไม่ระบุตัวเลข (spec-vs-code mismatch) — ปัจจุบันผู้ใช้งาน
> ยืนยันแล้วและบันทึกไว้ใน [Smart Integrations spec § ข้อสมมติฐาน/การตัดสินใจที่ยืนยันแล้ว](../../../01-requirements/01-spec/20260823-04-smart-integrations.md#ข้อสมมติฐานการตัดสินใจที่ยืนยันแล้ว)
> ว่าต้องมี log แคลอรี่สะสม **อย่างน้อย 7 วัน** (หนึ่งสัปดาห์เต็มตาม Weekly Planner ของ PLN-1) — น้อยกว่า 7
> วัน → empty state, ครบ 7 วันพอดี → พยากรณ์ได้ — `user-journeys.md` § INT-1 และโค้ดจริง
> (`MIN_LOG_DAYS_FOR_FORECAST = 7`) ปรับตรงกันแล้ว ไม่มี mismatch ค้างอยู่ (Risk R4 ใน test-plan.md ปิดแล้ว)
> — TC-INT-1-001/002 ด้านล่างใช้เกณฑ์ 7 วันเป็นค่าทางการ และเพิ่ม boundary **TC-INT-1-009** (6 วัน) /
> **TC-INT-1-010** (7 วันพอดี) ครอบคลุม AC-INT-1-08/09
...
- **Pre-condition**: ผู้ใช้ (persona ด้านบน) มีเป้าหมายน้ำหนัก 75.0 กก. บันทึกไว้ (จาก ONB-3) และมีประวัติ
  log แคลอรี่จริงสะสมจาก PLN-3 **อย่างน้อย 7 วัน** (เกณฑ์ทางการที่ resolve แล้ว 2026-09-26 — ดูหมายเหตุ
  ด้านบน) โดยอัตราขาดดุลเฉลี่ยที่คำนวณได้จาก log ไม่เป็น 0 และสอดคล้องกับทิศทางเป้าหมาย (ลดน้ำหนัก)
- **Test Steps**:
...
- **Test Data**: น้ำหนักปัจจุบัน 80.0 กก. เป้าหมาย 75.0 กก. ประวัติ log แคลอรี่เผาผลาญจริง **7 วัน**
  เฉลี่ย = 500 kcal/วัน (เช่น 500 kcal ทุกวันทั้ง 7 วัน) (ค่าอิสระ เลือกให้หารลงตัวพอดีกับตัวอย่างนี้ — **ไม่ผูกกับสูตร `dailyIntakeTargetKcal`
  (TDEE−500) อีกต่อไป** ตามหมายเหตุด้านบน) วันที่เปิดหน้าอ้างอิง = 27 ส.ค. 2569
- **References**: REQ-11 · AC-INT-1-01 · [user-journeys.md#int-1--พยากรณ์วันถึงเป้าหมายน้ำหนัก-req-11](../../../02-design/01-prototypes/user-journeys.md#int-1--พยากรณ์วันถึงเป้าหมายน้ำหนัก-req-11) ·
  ...
  โค้ดจริง `apps/web/server/domain/weightForecast.ts`

### TC-INT-1-002 — ประวัติ log น้อยกว่า 7 วัน (เกณฑ์ทางการ) แสดง empty state แทนวันที่พยากรณ์

- **Pre-condition**: ผู้ใช้เพิ่งผ่าน onboarding และตั้งเป้าหมายน้ำหนัก 75.0 กก. แต่ยังมีประวัติ log แคลอรี่
  จริงสะสมน้อยกว่า **7 วัน** ซึ่งเป็นเกณฑ์ขั้นต่ำทางการที่ resolve แล้ว 2026-09-26 (ตัวอย่างนี้: เพิ่งเริ่มใช้
  แอป 1 วัน — กรณีขอบเขต 6 วันทดสอบแยกที่ TC-INT-1-009)
- **Test Steps**:
  1. เปิดแอปและไปที่แท็บ "ความคืบหน้า"
  2. สังเกตเนื้อหาที่แสดงแทนการ์ดพยากรณ์
- **Expected Result**: ระบบไม่แสดงวันที่คาดการณ์ใด ๆ แต่แสดง empty state "ประวัติยังไม่พอสำหรับพยากรณ์"
  แจ้งว่าต้องสะสมข้อมูลเพิ่ม (เช่น "ยังต้องบันทึกผลอีกสักระยะก่อนเริ่มพยากรณ์ได้") พร้อมปุ่มพาไปบันทึกผล —
  ไม่มีวันที่ปลอม/ประมาณการใดแสดงขึ้นมา
- **Test Data**: เป้าหมายน้ำหนัก 75.0 กก. จำนวนวันที่มี log จริง = 1 วัน (< 7 วัน ตามเกณฑ์ทางการ)
- **References**: REQ-11 · AC-INT-1-02 · [user-journeys.md#int-1--พยากรณ์วันถึงเป้าหมายน้ำหนัก-req-11](../../../02-design/01-prototypes/user-journeys.md#int-1--พยากรณ์วันถึงเป้าหมายน้ำหนัก-req-11) ·
  ...
  demo toggle "ตัวอย่าง: ข้อมูลยังไม่พอ") · โค้ดจริง `apps/web/server/domain/weightForecast.ts`
  (`MIN_LOG_DAYS_FOR_FORECAST`)
...
> **หมายเหตุ spec-vs-code mismatch (พบ 2026-09-26, ยังไม่ resolve — flag ไว้แทนการเดา/ลบทิ้งเงียบๆ)**:
> AC-INT-1-03 (upstream) อธิบาย 2 กรณีย่อยรวมกัน คือ "อัตราขาดดุลเฉลี่ยเป็น 0 **หรือสวนทางเป้าหมาย**" — แต่
> ยืนยันจากโค้ดจริง `apps/web/server/domain/weightForecast.ts` แล้วพบว่า **กรณี "สวนทางเป้าหมาย" (ค่าติดลบ)
> เป็นไปไม่ได้จริงในโค้ดปัจจุบัน**: `averageDailyDeficitKcal` คำนวณจากค่าเฉลี่ยของ `accumulatedKcal`
> (แคลอรี่ที่**เผาผลาญจากการออกกำลังกายจริง**ต่อวัน จาก PLN-3) ซึ่งเป็นค่าที่**ไม่มีวันติดลบได้เลย** (ออก
> กำลังกายเผาผลาญ 0 กิโลแคลอรี่ได้อย่างมาก แต่ไม่มีทาง "ติดลบ") เพราะแอปยังไม่มีฟีเจอร์บันทึกอาหาร (intake
> logging) ที่จะทำให้เกิด "กินเกิน" ตามแนวคิดเดิมของ journey ได้จริง — โค้ดจึงเช็คแค่ `<= 0` (มีแค่กรณี "= 0"
> ที่เกิดขึ้นได้จริง ไม่มีกรณี "< 0") — **test case นี้จึงทดสอบ input ที่ไม่มีทางเกิดขึ้นจริงกับโค้ดปัจจุบัน**
> ไม่ได้ลบทิ้งเพราะยังคงมีคุณค่าเป็นเอกสารของเจตนาเดิมของ AC-INT-1-03/journey (เผื่อมีฟีเจอร์บันทึกอาหารใน
> อนาคตที่ทำให้ "สวนทางเป้าหมาย" เกิดขึ้นได้จริง) — **แนะนำให้ยืนยันกับผู้ใช้งาน/`feature-list-journey`**
> ว่าควร (ก) ถือว่า sub-case นี้ deprecated/ไม่เกี่ยวข้องแล้วตราบใดที่ยังไม่มี intake logging (ข) เก็บไว้เป็น
> forward-looking placeholder ต่อไป หรือ (ค) แก้ AC-INT-1-03 ให้ตรงกับพฤติกรรมจริง (มีแค่กรณี "= 0")
...
- **Test Data**: น้ำหนักปัจจุบัน 80.0 กก. เป้าหมาย 75.0 กก. (ทิศทาง = ลด) แต่อัตราเฉลี่ยจาก log จริงย้อนหลัง
  14 วัน = **ส่วนเกิน (surplus) +150 kcal/วัน** (สวนทางเป้าหมายลดน้ำหนัก — **ดูหมายเหตุด้านบน: input นี้ไม่
  เกิดขึ้นจริงกับโค้ดปัจจุบันที่ track เฉพาะแคลอรี่เผาผลาญจากการออกกำลังกาย ไม่มีทางติดลบ**)
- **References**: REQ-11 · AC-INT-1-03 (variation 2/2 — TC-INT-1-003 และ TC-INT-1-004 ครอบคลุมทั้งสองกรณี
  ย่อยของ AC-INT-1-03: "เป็น 0" และ "สวนทางเป้าหมาย" — กรณีหลัง unreachable ในโค้ดปัจจุบัน ดูหมายเหตุด้านบน) ·
  ...
  [10-progress-insights.html](../../../02-design/01-prototypes/v1/10-progress-insights.html) · โค้ดจริง
...
### TC-INT-1-009 — ขอบเขต: มี log 6 วัน (ต่ำกว่าเกณฑ์ 1 วัน) ยังแสดง empty state (เพิ่ม 2026-09-26)

- **Pre-condition**: ผู้ใช้ (persona ด้านบน) มีเป้าหมายน้ำหนัก 75.0 กก. บันทึกไว้ (จาก ONB-3) และมีประวัติ
  log แคลอรี่จริงจาก PLN-3 สะสม **6 วันพอดี** โดยอัตราขาดดุลเฉลี่ยไม่เป็น 0 และสอดคล้องกับทิศทางเป้าหมาย
  (ข้อมูลอื่นเหมือน TC-INT-1-001 ทุกประการ ต่างกันเพียงจำนวนวัน)
- **Test Steps**:
  1. เปิดแอปและไปที่แท็บ "ความคืบหน้า" (Progress/Insights)
  2. สังเกตเนื้อหาที่แสดงแทนการ์ดพยากรณ์
- **Expected Result**: ระบบแสดง empty state "ประวัติยังไม่พอสำหรับพยากรณ์" เหมือน TC-INT-1-002 — **ไม่แสดง
  วันที่คาดการณ์** แม้จะคำนวณอัตราขาดดุลเฉลี่ย (500 kcal/วัน) ได้แล้วก็ตาม เพราะ 6 < 7
- **Test Data**: น้ำหนักปัจจุบัน 80.0 กก. เป้าหมาย 75.0 กก. ประวัติ log แคลอรี่เผาผลาญจริง **6 วัน** วันละ
  500 kcal (เฉลี่ย 500 kcal/วัน) วันที่เปิดหน้าอ้างอิง = 27 ส.ค. 2569 → คาดหวัง empty state (ไม่มีวันที่)
- **References**: REQ-11 · AC-INT-1-08 · [user-journeys.md#int-1--พยากรณ์วันถึงเป้าหมายน้ำหนัก-req-11](../../../02-design/01-prototypes/user-journeys.md#int-1--พยากรณ์วันถึงเป้าหมายน้ำหนัก-req-11)
  (step 5, Alt/Edge Case "น้อยกว่า 7 วัน") · [10-progress-insights.html](../../../02-design/01-prototypes/v1/10-progress-insights.html)
  (สถานะ `insufficientState`) · โค้ดจริง `apps/web/server/domain/weightForecast.ts`
  (`MIN_LOG_DAYS_FOR_FORECAST = 7`, เงื่อนไข `length < MIN_LOG_DAYS_FOR_FORECAST`)

### TC-INT-1-010 — ขอบเขต: มี log ครบ 7 วันพอดี แสดงวันที่คาดการณ์ได้ (เพิ่ม 2026-09-26)

- **Pre-condition**: ผู้ใช้ (persona ด้านบน) มีเป้าหมายน้ำหนัก 75.0 กก. บันทึกไว้ (จาก ONB-3) และมีประวัติ
  log แคลอรี่จริงจาก PLN-3 สะสม **7 วันพอดี** (เท่ากับเกณฑ์ขั้นต่ำ) โดยอัตราขาดดุลเฉลี่ยไม่เป็น 0 และสอดคล้อง
  กับทิศทางเป้าหมาย
- **Test Steps**:
  1. เปิดแอปและไปที่แท็บ "ความคืบหน้า" (Progress/Insights)
  2. สังเกตค่าที่แสดงในการ์ดพยากรณ์
- **Expected Result**: ระบบแสดงวันที่คาดว่าจะถึงเป้าหมายน้ำหนัก (ไม่ใช่ empty state) — ยืนยันว่าเกณฑ์เป็น
  "≥ 7 วัน" ไม่ใช่ "> 7 วัน" — ค่าที่คำนวณเหมือน TC-INT-1-001: (80.0 − 75.0) × 7,700 ÷ 500 = **77 วัน** →
  เปิดหน้าวันที่ 27 ส.ค. 2569 แสดงวันที่คาดการณ์ = **12 พ.ย. 2569**
- **Test Data**: น้ำหนักปัจจุบัน 80.0 กก. เป้าหมาย 75.0 กก. ประวัติ log แคลอรี่เผาผลาญจริง **7 วัน** วันละ
  500 kcal (เฉลี่ย 500 kcal/วัน) วันที่เปิดหน้าอ้างอิง = 27 ส.ค. 2569 → คาดหวังวันที่ 12 พ.ย. 2569
- **References**: REQ-11 · AC-INT-1-09 · [user-journeys.md#int-1--พยากรณ์วันถึงเป้าหมายน้ำหนัก-req-11](../../../02-design/01-prototypes/user-journeys.md#int-1--พยากรณ์วันถึงเป้าหมายน้ำหนัก-req-11)
  (steps 4, 6–9, Alt/Edge Case "มีครบ 7 วันพอดี → พยากรณ์ได้") ·
  [10-progress-insights.html](../../../02-design/01-prototypes/v1/10-progress-insights.html) (สถานะ `forecastState`) ·
  โค้ดจริง `apps/web/server/domain/weightForecast.ts` (`MIN_LOG_DAYS_FOR_FORECAST = 7`)
...
- **Expected Result**: น้ำหนักในโปรไฟล์อัปเดตเป็น 79.2 กก. และองค์ประกอบร่างกาย (ไขมัน 24.5%) ถูกบันทึก
  ...
  2,662 kcal/วัน เดิม) **และ (ยืนยันจากโค้ดจริง `apps/web/server/routes/integration-gateway/index.ts`,
  เพิ่ม 2026-09-26)** `dailyCalorieTargetKcal` (เป้าหมายเผาผลาญของ ONB-3 ที่ REC-1/PLN-3/INT-1 ใช้จริง)
  ถูกคำนวณใหม่ในคำขอเดียวกันด้วยน้ำหนักใหม่: 79.2 × 4.5 = **356.4 kcal/วัน** (ลดลงจาก 360.0 kcal/วัน เดิม
  ที่คำนวณจากน้ำหนัก 80.0 กก.) — ค่าทั้งสอง (TDEE ใหม่ และ `dailyCalorieTargetKcal` ใหม่) ถูกใช้เป็น
  baseline ต่อให้ REC-1 (จับคู่วิดีโอ), REC-2 (คำนวณแคลอรี่เผาผลาญ), PLN-3 (all-or-nothing) และ INT-1
  (พยากรณ์) ในครั้งถัดไป
- **Test Data**: น้ำหนักก่อนซิงค์ 80.0 กก. → น้ำหนักหลังซิงค์จากตาชั่ง 79.2 กก., ไขมัน 24.5%, TDEE เดิม
  2,662 kcal/วัน → TDEE ใหม่ 2,650 kcal/วัน, `dailyCalorieTargetKcal` เดิม 360.0 kcal/วัน → ใหม่ 356.4
  kcal/วัน (เป้าหมาย = ลดน้ำหนัก, ×4.5)
...
| INT-1 | AC-INT-1-08 (ใหม่, เพิ่ม 2026-09-26, boundary 6 วัน) | TC-INT-1-009 | 1:1 |
| INT-1 | AC-INT-1-09 (ใหม่, เพิ่ม 2026-09-26, boundary 7 วันพอดี) | TC-INT-1-010 | 1:1 |
...
| **รวม** | **29 AC scenario ไม่ซ้ำ** (ครบทุก AC ของ INT-0/1/2/3 ใน acceptance-criteria.md — TC-INT-3-004 map ซ้ำกับ AC-INT-0-04) | **34 test case** | |

> อัปเดต 2026-09-26 (`test-suite-builder`, ขอบเขต INT-1 เท่านั้น): จำนวนวัน log ขั้นต่ำของ INT-1/REQ-11
> resolve แล้ว = **7 วัน** — ปรับ TC-INT-1-001 (pre-condition/test data 7 วัน เฉลี่ย 500 kcal) และ
> TC-INT-1-002 (7 วันเป็นเกณฑ์ทางการ) และเพิ่ม **TC-INT-1-009** (6 วัน → empty state) / **TC-INT-1-010**
> (7 วันพอดี → พยากรณ์ได้) ครอบคลุม **AC-INT-1-08/09** ใหม่ — AC ไม่ซ้ำจาก 27 เป็น **29**, test case จาก
> 32 เป็น **34**
