...
2 กลุ่ม: (1) test case ที่อ้างอิงวันที่/สถานะ log จากตัวอย่างสัปดาห์เดียวกับ prototype (จันทร์ 24 – อาทิตย์
30 ส.ค. 2026, วันนี้ = พฤหัสบดี 27 ส.ค. 2026) เพื่อความสอดคล้องกับ mockup ที่มีอยู่ — ได้แก่ PLN-1 ทั้งหมด,
PLN-2-001/002/006 (2) test case ที่ต้องการความแม่นยำระดับ % ของเป้าหมาย (boundary test) หรือ
ต้องใช้สถานะ "วันนี้" ที่ขัดกับ dataset ของกลุ่ม (1) จึงกำหนดเป้าหมายแคลอรี่ต่อวันและสถานะ log เป็นค่า
อิสระของตัวเอง แทนการยืมค่าจาก prototype — ได้แก่ PLN-3/PLN-4 ทั้งหมด และ **PLN-2-003/004/005** (ปรับเข้า
กลุ่มนี้เมื่อ 2026-08-28 เพราะ decision ใหม่จำกัดการทับ log/ยกเลิก Cheat/Rest Day ไว้เฉพาะ "วันนี้" เท่านั้น
ซึ่งชนกับสถานะ "วันนี้ = ครบเป้าหมาย 410 kcal" ของ dataset กลุ่ม (1) จึงต้องใช้ตัวเลขอิสระแทน) — ทั้งสอง
กลุ่มสอดคล้องกับกติกาทางธุรกิจใน REQ-08/09/10 เหมือนกัน ต่างกันแค่ตัวเลขที่เลือกใช้
...
AC: [AC-PLN-1-01, AC-PLN-1-02, AC-PLN-1-03](../../../01-requirements/acceptance-criteria.md#pln-1--ปฏิทินวางแผนรายสัปดาห์) ·
...
### TC-PLN-1-004 — วันที่ผ่านมาแล้วซึ่งมี log อยู่ก่อน เปิดได้เฉพาะแบบ read-only

| ฟิลด์ | รายละเอียด |
|---|---|
| Test Case Name | ผู้ใช้เปิดดูวันที่ผ่านมาแล้วในสัปดาห์เดียวกันซึ่งมี log อยู่ก่อน ต้องเห็นเป็น read-only แก้ไขไม่ได้ |
| Pre-condition | เปิดแท็บ Planner เห็นสัปดาห์ 24–30 ส.ค. 2026 วันนี้ = พฤหัสบดี 27 ส.ค. 2026 วันจันทร์ 24 ส.ค. เป็นวันที่ผ่านมาแล้วและมี log อยู่ก่อนแล้ว (260 kcal จากเป้าหมายวันนั้น 400 kcal สถานะ "ไม่ครบเป้าหมาย" — ตัวอย่างเดียวกับ TC-PLN-2-004/TC-PLN-4-001) |
| Test Steps | 1. เปิดแท็บ Planner/ปฏิทิน<br>2. แตะวันจันทร์ 24 ส.ค. (วันที่ผ่านมาแล้วและมี log)<br>3. สังเกตชีทรายละเอียดวันที่เปิดขึ้นมา และลองแตะช่องเลือกประเภทกิจกรรมหรือสวิตช์ Cheat/Rest Day |
| Expected Result | ระบบเปิดชีทรายละเอียดของวันจันทร์ 24 ส.ค. แบบ read-only เท่านั้น (แสดง log เดิม 260 kcal/ไม่ครบเป้าหมาย) ช่องเลือกประเภทกิจกรรมและสวิตช์ Cheat/Rest Day ถูกปิดใช้งาน (disabled) ไม่สามารถแก้ไขอะไรผ่านหน้านี้ได้ |
| Test Data | สัปดาห์ 24–30 ส.ค. 2026, วันนี้ = พฤหัสบดี 27 ส.ค. 2026, วันที่เปิดดู = จันทร์ 24 ส.ค. 2026 (ผ่านมาแล้ว, มี log = 260 kcal/400 kcal เป้าหมาย/ไม่ครบเป้าหมาย) |
| References | REQ-08 · AC-PLN-1-03 · [user-journeys.md#pln-1](../../../02-design/01-prototypes/user-journeys.md#pln-1--ปฏิทินวางแผนรายสัปดาห์-req-08) ขั้นตอน 3–4, Alt/Edge Case ข้อ 2 (resolved 2026-08-27) |

> **หมายเหตุความสอดคล้อง**: TC-PLN-1-004 ใช้ log ของวันจันทร์ 24 ส.ค. เดียวกับที่ TC-PLN-4-001 อ้างอิง
> (260 kcal, ไม่ครบเป้าหมาย) — ตรวจสอบแล้วว่าค่านี้ตรงกับ dataset ล่าสุดที่ `prototype-builder` เพิ่งปรับ
> `v1/08-weekly-planner.html`/`v1/09-log-history.html` ให้ตรงกันเมื่อ 2026-08-28 ไม่ใช่ค่าเก่าที่หลุด sync
> วันจันทร์ 24 ส.ค. เดียวกันนี้ยังถูกใช้ซ้ำใน TC-PLN-2-006 ด้านล่าง เพื่อยืนยันว่า read-only ของ PLN-1 ปิดกั้น
> การตั้ง Cheat/Rest Day ของ PLN-2 ด้วยเช่นกัน (ไม่มีข้อยกเว้น)
>
> **resolve แล้วเมื่อ 2026-08-28**: คำถามที่เคยค้างเรื่อง TC-PLN-2-003/004 (ซึ่งเดิมทดสอบการ "ตั้ง
> Cheat/Rest Day ย้อนหลัง" ผ่านปฏิทินบนวันอังคาร 25 ส.ค./วันจันทร์ 24 ส.ค. ที่เป็นวันในอดีตของสัปดาห์และ
> ขัดกับ read-only ของ PLN-1 ข้างต้น) ได้รับคำตอบจากผู้ใช้แล้ว: **เลือกตัวเลือก (A) จำกัดเฉพาะวันนี้** —
> การตั้ง/เปลี่ยน Cheat/Rest Day ทับ log ที่มีอยู่แล้วทำได้เฉพาะ "วันนี้" เท่านั้น บันทึกไว้ใน
...
AC: [AC-PLN-2-01, AC-PLN-2-02, AC-PLN-2-03, AC-PLN-2-04](../../../01-requirements/acceptance-criteria.md#pln-2--โหมด-cheat-day--rest-day-preserve-log-completed-wins) ·
...
### TC-PLN-2-003 — ตั้ง Rest Day ทับ log ของวันนี้ที่ครบเป้าหมายอยู่แล้ว (จำกัดเฉพาะวันนี้)

> **แก้ไขเมื่อ 2026-08-28**: เดิม test case นี้ใช้วันอังคาร 25 ส.ค. (วันในอดีตของสัปดาห์) ซึ่งขัดกับ
> decision ใหม่ของ PLN-1 ที่ทำให้วันในอดีตที่มี log กลายเป็น read-only (ดู TC-PLN-1-004) — resolve แล้วว่า
> การทับ log ด้วย Cheat/Rest Day ทำได้เฉพาะ **"วันนี้"** เท่านั้น จึงย้ายมาใช้ "วันนี้" แทน และเปลี่ยนไปใช้
> ตัวเลขอิสระ (ไม่ใช่ dataset สัปดาห์ 24–30 ส.ค.) เพราะ "วันนี้" ในชุดข้อมูลนั้น (พฤหัสบดี 27 ส.ค.) ถูกกำหนด
> ไว้แล้วว่าครบเป้าหมายด้วย log จริง 410 kcal (ดู TC-PLN-4-001) — ใช้ตัวเลขอื่นเพื่อไม่ให้ชนกับ dataset นั้น

| ฟิลด์ | รายละเอียด |
|---|---|
| Test Case Name | ตั้ง Cheat/Rest Day ทับ log ของวันนี้ที่ครบเป้าหมายอยู่แล้ว ต้องเก็บ log เดิมไว้ — ทำได้เฉพาะวันนี้เท่านั้น |
| Pre-condition | วันนี้มีเป้าหมายแคลอรี่ = 500 kcal (test data อิสระ แยกจาก dataset สัปดาห์ 24–30 ส.ค. ของ TC-PLN-1/TC-PLN-4) และมี log อยู่แล้วจากการออกกำลังกายจริงในช่วงเช้า: เวทเทรนนิ่ง 30 นาที, 500 kcal (100%), สถานะ "ครบเป้าหมาย" |
| Test Steps | 1. เปิด Daily Dashboard ของวันนี้ (หรือแตะวันนี้ในปฏิทินรายสัปดาห์)<br>2. เปิดสวิตช์ "Cheat Day / Rest Day"<br>3. กด "บันทึก" |
| Expected Result | เนื่องจากเป็นวันนี้ (ไม่ใช่วันในอดีตของสัปดาห์) ระบบอนุญาตให้ทับ log ได้ — เก็บ log เดิม (เวทเทรนนิ่ง 30 นาที, 500 kcal) ไว้ทั้งหมด ไม่ลบทิ้ง และสถานะวันนั้นยังคง "ครบเป้าหมาย" (completed) เหมือนเดิม |
| Test Data | วันที่ตั้ง = วันนี้ (test data อิสระ, เป้าหมาย = 500 kcal), log เดิม = เวทเทรนนิ่ง 30 นาที / 500 kcal (100%) / สถานะครบเป้าหมาย, ประเภทที่ตั้งใหม่ = Rest Day |
| References | REQ-09 · AC-PLN-2-02 · [user-journeys.md#pln-2](../../../02-design/01-prototypes/user-journeys.md#pln-2--โหมด-cheat-day--rest-day-req-09) ขั้นตอน 6–7 (resolved 2026-08-28: จำกัดเฉพาะวันนี้) |

### TC-PLN-2-004 — ตั้ง Cheat Day ทับ log ของวันนี้ที่ "ไม่ครบเป้าหมาย" (completed ชนะเสมอ, จำกัดเฉพาะวันนี้)

> **แก้ไขเมื่อ 2026-08-28**: เดิม test case นี้ใช้วันจันทร์ 24 ส.ค. (วันในอดีตของสัปดาห์ที่มี log 260 kcal/
> ไม่ครบเป้าหมาย) — resolve แล้วว่าต้องเป็น "วันนี้" เท่านั้นเช่นเดียวกับ TC-PLN-2-003 (เหตุผลเดียวกัน) วันจันทร์
> 24 ส.ค. เดิมยังคงใช้ต่อใน `TC-PLN-1-004` และ `TC-PLN-2-006` ใหม่ด้านล่างแทน เพื่อยืนยันฝั่งที่ถูกปิดกั้น

| ฟิลด์ | รายละเอียด |
|---|---|
| Test Case Name | ตั้ง Cheat/Rest Day ทับ log เดิมของวันนี้ที่สถานะ "ไม่ครบเป้าหมาย" ต้องเปลี่ยนสถานะเป็นครบเป้าหมายเสมอ (completed ชนะเสมอ) — ทำได้เฉพาะวันนี้เท่านั้น |
| Pre-condition | วันนี้มีเป้าหมายแคลอรี่ = 500 kcal (test data อิสระ เช่นเดียวกับ TC-PLN-2-003 — ไม่ผูกกับ dataset สัปดาห์ 24–30 ส.ค.) และมี log อยู่แล้วจากการออกกำลังกายจริงในช่วงเช้า: 350 kcal (70%) สถานะ "ไม่ครบเป้าหมาย" |
| Test Steps | 1. เปิด Daily Dashboard ของวันนี้ (หรือแตะวันนี้ในปฏิทินรายสัปดาห์)<br>2. เปิดสวิตช์ "Cheat Day / Rest Day" (เลือก Cheat Day)<br>3. กด "บันทึก" |
| Expected Result | เนื่องจากเป็นวันนี้ ระบบอนุญาตให้ทับ log ได้ — เก็บ log เดิม (350 kcal) ไว้ไม่ลบทิ้ง แต่เปลี่ยนสถานะของวันนั้นจาก "ไม่ครบเป้าหมาย" เป็น "ครบเป้าหมาย" (completed) ทันที ตามกติกา "completed ชนะเสมอ" — Cheat/Rest Day ที่ตั้งในวันนี้ไม่ทำให้สถานะแย่ลงหรือคงสถานะเดิมที่แย่กว่าไว้ |
| Test Data | วันที่ตั้ง = วันนี้ (test data อิสระ, เป้าหมาย = 500 kcal), log เดิม = 350 kcal / สถานะเดิม "ไม่ครบเป้าหมาย", ประเภทที่ตั้งใหม่ = Cheat Day |
| References | REQ-09 · AC-PLN-2-02 · [user-journeys.md#pln-2](../../../02-design/01-prototypes/user-journeys.md#pln-2--โหมด-cheat-day--rest-day-req-09) ขั้นตอน 6–7 (resolved 2026-08-28: จำกัดเฉพาะวันนี้) (หมายเหตุ: เป็น test-data variation เพิ่มเติมของ AC-PLN-2-02 นอกเหนือจาก TC-PLN-2-003 — กรณีนี้สำคัญกว่าเพราะพิสูจน์ว่า "completed ชนะเสมอ" ทำงานแม้ log เดิมจะเป็นสถานะแย่ที่สุดก็ตาม) |

### TC-PLN-2-006 — พยายามตั้ง Cheat/Rest Day ทับ log ของวันในอดีตของสัปดาห์ ถูกปิดกั้น (ไม่มีข้อยกเว้น)

| ฟิลด์ | รายละเอียด |
|---|---|
| Test Case Name | ผู้ใช้พยายามตั้ง Cheat/Rest Day ทับ log ของวันในอดีตของสัปดาห์ผ่านปฏิทิน ต้องถูกปิดกั้น (read-only ตาม PLN-1 ไม่มีข้อยกเว้นสำหรับ PLN-2) |
| Pre-condition | เปิดแท็บ Planner เห็นสัปดาห์ 24–30 ส.ค. 2026 วันนี้ = พฤหัสบดี 27 ส.ค. 2026 วันจันทร์ 24 ส.ค. เป็นวันในอดีตของสัปดาห์และมี log อยู่ก่อนแล้ว (260 kcal จากเป้าหมาย 400 kcal สถานะ "ไม่ครบเป้าหมาย" — ตัวอย่างเดียวกับ TC-PLN-1-004/TC-PLN-4-001) |
| Test Steps | 1. เปิดปฏิทินรายสัปดาห์ แตะวันจันทร์ 24 ส.ค.<br>2. สังเกตชีทรายละเอียดวันที่เปิดขึ้นมา และลองแตะสวิตช์ "Cheat Day / Rest Day" |
| Expected Result | ระบบเปิดชีทรายละเอียดของวันจันทร์ 24 ส.ค. แบบ read-only เท่านั้น (ตาม PLN-1) สวิตช์ Cheat Day/Rest Day ถูกปิดใช้งาน (disabled) ไม่สามารถตั้ง/เปลี่ยนได้ ไม่มีข้อยกเว้นสำหรับ PLN-2 — log เดิม (260 kcal, ไม่ครบเป้าหมาย) ยังคงสถานะเดิมไม่เปลี่ยนแปลง |
| Test Data | สัปดาห์ 24–30 ส.ค. 2026, วันนี้ = พฤหัสบดี 27 ส.ค. 2026, วันที่พยายามตั้ง = จันทร์ 24 ส.ค. 2026 (ผ่านมาแล้ว, มี log = 260 kcal/400 kcal เป้าหมาย/ไม่ครบเป้าหมาย) |
| References | REQ-09 · AC-PLN-2-04 · [user-journeys.md#pln-2](../../../02-design/01-prototypes/user-journeys.md#pln-2--โหมด-cheat-day--rest-day-req-09) ขั้นตอน 2, Alt/Edge Case ข้อ 2 (resolved 2026-08-28) · เชื่อมกับ [TC-PLN-1-004](#pln-1--ปฏิทินวางแผนรายสัปดาห์) (พฤติกรรม read-only เดียวกันของวันจันทร์ 24 ส.ค.) |

### TC-PLN-2-005 — ยกเลิกการตั้ง Cheat/Rest Day ของวันนี้ก่อนสิ้นวัน

> **แก้ไขเมื่อ 2026-08-28**: เดิม test case นี้ใช้วันพุธ 26 ส.ค. ซึ่งเป็นวันในอดีตของสัปดาห์เทียบกับ "วันนี้
> = พฤหัสบดี 27 ส.ค." ของ dataset เดียวกัน ทั้งที่ pre-condition ระบุว่า "ยังไม่สิ้นวัน" (สื่อถึงวันปัจจุบัน) —
> เป็นความคลาดเคลื่อนที่ค้างมาตั้งแต่สร้างเอกสารครั้งแรก แก้ให้ใช้ "วันนี้" แทน และย้ายไปใช้ test data อิสระ
> (ไม่ผูกกับ dataset สัปดาห์ 24–30 ส.ค.) เช่นเดียวกับ TC-PLN-2-003/004 เพราะ "วันนี้" ของ dataset นั้น
> (27 ส.ค.) ถูกกำหนดไว้แล้วว่ามี log จริงครบเป้าหมาย 410 kcal (TC-PLN-4-001) ซึ่งขัดกับสถานะ "ตั้งเป็น Rest
> Day ไว้ก่อน ยังไม่มี log ออกกำลังกายจริง" ที่ test case นี้ต้องการ

| ฟิลด์ | รายละเอียด |
|---|---|
| Test Case Name | ผู้ใช้ยกเลิกการตั้ง Rest Day ของวันนี้ก่อนสิ้นวัน กลับไปนับเป้าหมายแคลอรี่ตามปกติ |
| Pre-condition | วันนี้ถูกตั้งเป็น Rest Day อยู่ก่อนแล้ว (สถานะ "ครบเป้าหมาย" จาก PLN-2, ยังไม่มี log ออกกำลังกายจริงเกิดขึ้น) ยังไม่สิ้นวัน (test data อิสระ แยกจาก dataset สัปดาห์ 24–30 ส.ค. — เหตุผลเดียวกับ TC-PLN-2-003/004) |
| Test Steps | 1. เปิด Daily Dashboard ของวันนี้ (หรือแตะวันนี้ในปฏิทินรายสัปดาห์)<br>2. ปิดสวิตช์ "Cheat Day / Rest Day"<br>3. กด "บันทึก" |
| Expected Result | วันนี้กลับไปนับเป้าหมายแคลอรี่ตามปกติ (ไม่ใช่ completed อัตโนมัติอีกต่อไป) และสถานะของวันนั้นจะถูกประเมินใหม่ตามกติกา all-or-nothing ของ PLN-3 เมื่อมีการออกกำลังกายจริงหรือถึงสิ้นวัน |
| Test Data | วันที่ยกเลิก = วันนี้ (test data อิสระ), สถานะก่อนยกเลิก = Rest Day (ครบเป้าหมาย, ยังไม่มี log จริง), การกระทำ = ปิดสวิตช์ Cheat/Rest Day ก่อนสิ้นวัน |
